...
main(void)
{
    ...
    const std::string file_name("cmc_out_test_multi_res_patch_lossless_idw_serial.cmc");

    cmc::patch::lossless::multi_res::CompressionVariable<float, DIM> compression_variable(initial_data, kDimLength);
 
    compression_variable.Compress();

    compression_variable.WriteCompressedData(file_name.c_str());

    cmc::patch::lossless::multi_res::ReadCompressionInfo(file_name);
  
    cmc::patch::lossless::multi_res::DecompressionVariable<float, DIM> decompression_variable(file_name);
    ...
    for (int32_t elem_idx{0}; elem_idx < num_elems; ++elem_idx)
    {
        if (init_data[elem_idx] != decompressed_data[elem_idx])
        {
            ...
            cmc::cmc_debug_msg("Init Bitset: ", std::bitset<32>(init_data[elem_idx]), ", Decompressed Bitset: ", std::bitset<32>(decompressed_data[elem_idx]));
        }
