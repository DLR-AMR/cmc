...
#include "amr/lossy/cmc_par_multi_res_extraction_idw.hxx"
#include "amr/lossy/cmc_par_multi_res_decompression_idw.hxx"
           ...
           [[maybe_unused]] t8_element_t * elements[])
{
    ...
    if ((gtree_id == 3 || gtree_id == 7 || gtree_id == 10) && is_family) {return -1;}
    if (gtree_id == 4) {return 1;}
...
CreateInitMesh()
{
    ...
    t8_cmesh_t cmesh;
    cmesh = t8_cmesh_new_hypercube_hybrid (MPI_COMM_WORLD, 0, 0);
...
{
    t8_cmesh_t cmesh = t8_cmesh_new_hypercube_hybrid (MPI_COMM_WORLD, 0, 0);
...
CreateInitData(const t8_forest_t mesh)
{
    ...
    for (t8_locidx_t itree = 0, idata = 0; itree < num_trees; itree++)
    {
        ...
        for (t8_locidx_t ielement = 0; ielement < num_elems_in_tree; ielement++)
        {
            ...
            data.push_back(std::cos(elem_coords[0] - 0.5) * std::cos(elem_coords[1] - 0.5) * std::cos(elem_coords[2] - 0.5));
        }
    }
...
main(void)
{
    ...
    cmc::CmcInitialize();
    {
        ...
        const std::string file_name("cmc_out_test_multi_res_idw_lossy_hybrid_mesh_serial.cmc");
        ...
        cmc::par::lossy::multi_res::idw::CompressionVariable<float, DIM> variable("test_var", mesh, init_data, error_domains);
        ...
        //Decompression
        {
        cmc::par::lossy::multi_res::idw::ReadCompressionInfo(file_name);
        
        /* Get the initial base mesh */
        const auto [cmesh, scheme] = cmc::test::GetInitCMeshAndScheme();

        /* Decompress the mesh and the data */
        cmc::par::lossy::multi_res::idw::DecompressionVariable<float, DIM> decompression_variable(file_name, MPI_COMM_WORLD, cmesh, scheme);
