...
#include "patch/lossy/cmc_multi_res_decompression_idw.hxx"
...
main(void)
{
    ...
    const std::string file_name("cmc_out_test_multi_res_patch_lossy_idw_serial.cmc");

    cmc::patch::lossy::multi_res::idw::CompressionVariable<float, DIM> compression_variable(initial_data, kDimLength, permitted_abs_error);
 
    compression_variable.Compress();

    compression_variable.WriteCompressedData(file_name.c_str());

    cmc::patch::lossy::multi_res::idw::ReadCompressionInfo(file_name);
  
    cmc::patch::lossy::multi_res::idw::DecompressionVariable<float, DIM> decompression_variable(file_name);
