    ...
    switch (dim)
    {
        ...
        case 2:
            switch (data_type)
            {
                ...
                {
                    cmc::patch::lossy::multi_res::idw::DecompressionVariable<float, 2> decompression_variable(file_name);
                ...
                {
                    cmc::patch::lossy::multi_res::idw::DecompressionVariable<double, 2> decompression_variable(file_name);
                    decompression_variable.Decompress();
        ...
        case 3:
            switch (data_type)
            {
                case cmc::CmcType::Float:
                {
                    cmc::patch::lossy::multi_res::idw::DecompressionVariable<float, 3> decompression_variable(file_name);
                    decompression_variable.Decompress();
                    const std::vector<float> decompressed_data = decompression_variable.GetDecompressedData();
                    WriteOutDecompressedData<float>(output_file, decompressed_data);
                }
                ...
                case cmc::CmcType::Double:
                {
                    cmc::patch::lossy::multi_res::idw::DecompressionVariable<double, 3> decompression_variable(file_name);
                    decompression_variable.Decompress();
                    const std::vector<double> decompressed_data = decompression_variable.GetDecompressedData();
                    WriteOutDecompressedData<double>(output_file, decompressed_data);
                }
