...
CommunicateOffsets(cmc::lossless::embedded::AbstractEmbeddedByteCompressionVariable<T>* variable, const MPI_Comm comm)
{
...
template<typename T>
void
Writer::SetDataVariable(cmc::lossless::embedded::AbstractEmbeddedByteCompressionVariable<T>* variable, const std::vector<VariableLevelOffset>& level_byte_counts, const std::vector<IntraLevelStreamOffset>& intra_level_offsets, const int var_id, const int corresponding_mesh_id)
{
    ...
    attributes.reserve(num_compression_iterations + 9);
    ...
    attributes.emplace_back(missing_value_attr, static_cast<T>(variable->GetVariableAttributes().GetMissingValue()));
    attributes.emplace_back(initial_data_layout_attr, static_cast<int32_t>(variable->GetVariableAttributes().GetInitialDataLayout()));
    attributes.emplace_back(pre_compression_layout_attr, static_cast<int32_t>(variable->GetVariableAttributes().GetPreCompressionLayout()));
    attributes.emplace_back(global_context_information_attr, static_cast<int32_t>(variable->GetVariableAttributes().GetGlobalContextInformation()));
...
template<typename T>
void
Writer::SetMeshVariable(cmc::lossless::embedded::AbstractEmbeddedByteCompressionVariable<T>* variable, const std::vector<VariableLevelOffset>& level_byte_counts, const std::vector<IntraLevelStreamOffset>& intra_level_offsets, const int mesh_id)
{
    ...
    /* Check, if we do have encoded mesh refinement bits */
    const bool are_mesh_refinement_bits_given = variable->AreMeshRefinementBitsStored();
    ...
    if (are_mesh_refinement_bits_given)
    {
    ...
    {
        /* Otherwise, only the root level is of interest */
        global_byte_count += level_byte_counts.back().encoded_mesh_count;
    }
    ...
    /* Get the number of compression iterations */
    const int num_compression_iterations = static_cast<int32_t>(levelwise_encoded_mesh.size());

    /* Allocate the attributes vector */
    std::vector<cmc::nc::Attribute> attributes;

    /* Now, we define some attributes that will be added to the variable */
    attributes.reserve(3 + (are_mesh_refinement_bits_given ? num_compression_iterations : 0));

    /* Store some general information */
    attributes.emplace_back(id_attr, mesh_id);
    attributes.emplace_back(are_refinement_bits_stored_attr, static_cast<int32_t>(are_mesh_refinement_bits_given));
    attributes.emplace_back(num_compression_iterations_attr, num_compression_iterations);
    ...
    {
        /* Set only the root level data */
        const int root_lvl_idx = num_compression_iterations - 1;

        /* Get the intra-level offset based on the rank of this process */
        const uint64_t intra_level_offset = intra_level_offsets[root_lvl_idx].mesh_encoding_offset;

        /* Set the data of the current level with the correct offset */
        spec_variable.SetData(std::move(levelwise_encoded_mesh[root_lvl_idx]), intra_level_offset);
    }
...
template<typename T>
void
Writer::SetVariable(cmc::lossless::embedded::AbstractEmbeddedByteCompressionVariable<T>* variable)
{
...
#endif /* !CMC_EMBEDDED_BYTE_COMPRESSION_OUTPUT_TXX */
