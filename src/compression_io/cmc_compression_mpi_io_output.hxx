...
Writer<T>::GetDefaultFileHeaderOffset() const
{
    return static_cast<MPI_Offset>(DefaultFileHeader<T>::GetFileHeaderSize(variable_infos_.size));
}
...
#if 1
// Test alternative where only the actual variable data is written in parallel
template <typename T>
std::vector<MPI_Offset>
Writer<T>::ExchangeLocalInformationStreamLenghts(const MPI_Comm comm, const int comm_rank, const int comm_size)
{
    /* Count the global offsets */
    size_t count_local_offsets{0};
    for (auto var_info_iter = variable_infos_.begin(); var_info_iter != variable_infos_.end(); ++var_info_iter)
    ...
    for (auto var_info_iter = variable_infos_.begin(); var_info_iter != variable_infos_.end(); ++var_info_iter)
    {
        uint64_t additional_root_chunk_length{0};
        /* On the root rank we have a rather larg chunk of data (e.g. the partition table, mesh encoding and the start of the first data level).
         * This data amount needs to collected first */
        if (comm_rank == kRootRank)
        {
            /* Add all bytes for the partition table header */
            additional_root_chunk_length += var_info_iter->GetVariablePartitionTableHeader().size();
            /* Accumulate all bytes for the mesh encoding */
            additional_root_chunk_length += std::accumulate(var_info_iter->GetVariable().GetEncodedPartitionTable().begin(), var_info_iter->GetVariable().GetEncodedPartitionTable().end(), 0);

            /* Add all bytes for the mesh header */
            additional_root_chunk_length += var_info_iter->GetVariableMeshHeader().size();
            /* Accumulate all bytes for the mesh encoding */
            additional_root_chunk_length += std::accumulate(var_info_iter->GetVariable().GetEncodedMesh().begin(), var_info_iter->GetVariable().GetEncodedMesh().end(), 0);
        
            /* And additionally, the variable header */
            additional_root_chunk_length += var_info_iter->GetVariableDataHeader().size();
        }
        
        /* Get the encoded data */
        const auto data_encoding = var_info_iter->GetVariable().GetEncodedData();

        /* Now, we do need to iterate through the levels of the data encoding and store the encoding length */
        for (auto de_iter = data_encoding.rbegin(); de_iter != data_encoding.rend(); ++de_iter)
        {
            /* Get the length of this local level mesh encoding */
            SizeType data_lvl_encoding_length = de_iter->size();
            /* In case of the first iteration and the root rank, we add up additionally the header for the mesh variable */
            if (de_iter == data_encoding.rbegin() && comm_rank == kRootRank)
            {
                data_lvl_encoding_length += additional_root_chunk_length;
            }
...
Writer<T>::ComputeLocalChunkOffsets(const int comm_rank, const int comm_size, const SizeType num_file_header_bytes, const std::vector<SizeType>& global_information_lengths) const
{
    ...
    const int skip_to_next_process_same_var = local_num_data_offsets;

    /* Allocate an output vector for the write data offsets */
    std::vector<MPI_Offset> data_offsets;
    data_offsets.reserve(local_num_data_offsets);
    ...
    for (auto var_info_iter = variable_infos_.begin(); var_info_iter != variable_infos_.end(); ++var_info_iter)
    {
        ...
        /* Move the accessor to the start of the next variable */
        global_var_offset_accessor_id += num_data_levels;
    }

    return std::move(data_offsets);
}
...
Writer<T>::Write()
{
    ...
    /* We exchange all information byte stream lengths for the local processes to be written out globally in order to compute the file offsets for each process */
    const std::vector<SizeType> all_local_stream_lengths = this->ExchangeLocalInformationStreamLenghts(comm_, rank, size);

    /* Get the length of the file header */
    const MPI_Offset general_file_header_size = this->GetDefaultFileHeaderOffset();

    /* Compute for each rank, where the data has to be written */
    const std::vector<MPI_Offset> data_offsets = this->ComputeLocalChunkOffsets(rank, size, general_file_header_size, all_local_stream_lengths);

    cmc_debug_msg("Num data offsets: ", data_offsets.size());
    for (auto miter = data_offsets.begin(); miter != data_offsets.end(); ++miter)
    {
        cmc_debug_msg("Data Offset: ", *miter);
    }
    ...
    cmc_debug_msg("General file header size: ", general_file_header_size);

    /* Afterwards, we compute the global file storage to store all of the data */
    const MPI_Offset file_size = this->ComputeGlobalFileSizeFromLocalStreamLengths(general_file_header_size, all_local_stream_lengths);
    ...
    /* Iterate over the variables and store the process-local information chunks in the global file */
    int accessor_data_chunk{0};
    for (auto var_info_iter = variable_infos_.begin(); var_info_iter != variable_infos_.end(); ++var_info_iter)
    {
        /* Get the encoded data */
        const auto data_encoding = var_info_iter->GetVariable().GetEncodedData();
        ...
        for (auto data_lvl_iter = data_encoding.rbegin(); data_lvl_iter != data_encoding.rend(); ++data_lvl_iter, ++accessor_data_chunk)
        {
            ...
            if (data_lvl_iter == data_encoding.rbegin() && rank == kRootRank)
            {
                ...
                /* We buffer the information to send out */
                const size_t buffer_size = std::accumulate(var_info_iter->GetVariable().GetEncodedPartitionTable().begin(), var_info_iter->GetVariable().GetEncodedPartitionTable().end(), 0)
                                         + std::accumulate(var_info_iter->GetVariable().GetEncodedMesh().begin(), var_info_iter->GetVariable().GetEncodedMesh().end(), 0)
                                         + var_info_iter->GetVariablePartitionTableHeader().size()
                                         + var_info_iter->GetVariableMeshHeader().size()
                                         + var_info_iter->GetVariableDataHeader().size()
                                         + var_info_iter->GetVariable().GetEncodedData().rbegin()->size();
                
                cmc_debug_msg("Computed buffer size on root rank: ", buffer_size);

                std::vector<uint8_t> write_buffer;
                write_buffer.reserve(buffer_size);

                /* Copy the partition header */
                std::copy_n(var_info_iter->GetVariablePartitionTableHeader().begin(), var_info_iter->GetVariablePartitionTableHeader().size(), std::back_inserter(write_buffer));
                /* Copy the partition table */
                std::copy_n(var_info_iter->GetVariable().GetEncodedPartitionTable().begin(), var_info_iter->GetVariable().GetEncodedPartitionTable().size(), std::back_inserter(write_buffer));

                /* Copy the mesh encoding header */
                std::copy_n(var_info_iter->GetVariableMeshHeader().begin(), var_info_iter->GetVariableMeshHeader().size(), std::back_inserter(write_buffer));
                /* Copy the mesh encoding */
                std::copy_n(var_info_iter->GetVariable().GetEncodedMesh().begin(), var_info_iter->GetVariable().GetEncodedMesh().size(), std::back_inserter(write_buffer));
                
                /* Copy the variable header */
                std::copy_n(var_info_iter->GetVariableDataHeader().begin(), var_info_iter->GetVariableDataHeader().size(), std::back_inserter(write_buffer));

                /* Copy the local part of the root level encoding */
                std::copy_n(data_lvl_iter, data_lvl_iter->size(), std::back_inserter(write_buffer));

                cmc_debug_msg("Actual buffer size on root rank: ", write_buffer.size());
                
                MPI_Status status;

                /* Write the header */
                const int rv_write_data_buffer = MPI_File_write(this->fhandle_, write_buffer.data(), write_buffer.size(), MPI_UINT8_T, &status);
                MPICheckError(rv_write_data_buffer);
                CheckMPIWriteCorrectness(&status, MPI_UINT8_T, static_cast<int>(write_buffer.size()));
