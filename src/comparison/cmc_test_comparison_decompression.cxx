...
SetUpByteVariableForDecompression(const std::vector<CompressedVariableInfo>& var_info)
{
    ...
    for (auto var_iter = var_info.begin(); var_iter != var_info.end(); ++var_iter)
    {
        ...
        cmc_debug_msg("Domain:\n Dim: ", var_iter->domain.GetDimensionality(),"\n",
        "Lon Start: ", var_iter->domain.GetDimensionStartIndex(Dimension::Lon), ", Lon End: ", var_iter->domain.GetDimensionEndIndex(Dimension::Lon), "\n",
        "Lon Start: ", var_iter->domain.GetDimensionStartIndex(Dimension::Lon), ", Lon End: ", var_iter->domain.GetDimensionEndIndex(Dimension::Lon), "\n",
        "Lon Start: ", var_iter->domain.GetDimensionStartIndex(Dimension::Lon), ", Lon End: ", var_iter->domain.GetDimensionEndIndex(Dimension::Lon), "\n",
        "Lon Start: ", var_iter->domain.GetDimensionStartIndex(Dimension::Lon), ", Lon End: ", var_iter->domain.GetDimensionEndIndex(Dimension::Lon), "\n"
        );

        const int dimensionality = GetDimensionalityOfDataLayout(var_iter->initial_layout);

        /* Create the base mesh for this variable */
        t8_forest_t base_mesh = ReconstructBaseMesh(dimensionality, MPI_COMM_WORLD);

        const int initial_refinement_level = var_iter->intial_refinement_level;

        /* Assign the base mesh to the byte variable */
        byte_variables.back().SetMesh(base_mesh);
        byte_variables.back().GetAmrMesh().SetDimensionality(dimensionality);
        byte_variables.back().GetAmrMesh().SetInitialRefinementLevel(initial_refinement_level);
...
CreateDecompressionByteVariable(const CompressedVariableInfo& var_info, const std::string& name, const int split_id)
{
    ...
    cmc_debug_msg("Domain:\n Dim: ", var_info.domain.GetDimensionality(),"\n",
        "Lon Start: ", var_info.domain.GetDimensionStartIndex(Dimension::Lon), ", Lon End: ", var_info.domain.GetDimensionEndIndex(Dimension::Lon), "\n",
        "Lon Start: ", var_info.domain.GetDimensionStartIndex(Dimension::Lat), ", Lon End: ", var_info.domain.GetDimensionEndIndex(Dimension::Lat), "\n",
        "Lon Start: ", var_info.domain.GetDimensionStartIndex(Dimension::Lev), ", Lon End: ", var_info.domain.GetDimensionEndIndex(Dimension::Lev), "\n",
        "Lon Start: ", var_info.domain.GetDimensionStartIndex(Dimension::Time), ", Lon End: ", var_info.domain.GetDimensionEndIndex(Dimension::Time), "\n"
        );

    const int dimensionality = GetDimensionalityOfDataLayout(var_info.initial_layout);
    /* Create the base mesh for this variable */
    t8_forest_t base_mesh = ReconstructBaseMesh(dimensionality, MPI_COMM_WORLD);

    const int initial_refinement_level = var_info.intial_refinement_level;

    /* Assign the base mesh to the byte variable */
    var.SetMesh(base_mesh);
    var.GetAmrMesh().SetDimensionality(dimensionality);
    var.GetAmrMesh().SetInitialRefinementLevel(initial_refinement_level);
...
Decompressor::Setup()
{
    ...
    {
        cmc_err_msg("The file does not hold diff-comrpessed data. Therefore, a decompression cannot be appllied.");
    }
...
Decompressor::DecompressVariable(const std::string& variable_base_name)
{
    ...
    for (int time_step = 0; time_step < num_time_steps; ++time_step)
    {
        for (int split_id = 0; split_id < num_split_variables; ++split_id)
        {
            ...
            cmc_debug_msg("initial refinement level: ", initial_refinement_level);
            cmc_debug_msg("initial refinement level in amr mesh of variable: ", compression_variables_.back().GetAmrMesh().GetInitialRefinementLevel());
            /* Create an adapt data class which handles the compressed byte stream */
            _TestLightAMRPCPDecompressData adapt_data(compression_variables_.back(), serialized_data);

            /* Decode the levels of the prefix encodings */
            for (int lvl = 0; lvl < initial_refinement_level; ++lvl)
            {
                cmc_debug_msg("Diff Decompression iteration: ", lvl);
                
                /* Initialize the decompression iteration by sufficiently allocating memory */
                adapt_data.InitializeDiffDecompression();
                cmc_debug_msg("Initialization finished");
                /* Decompress the next level */
                t8_forest_t decompressed_forest = t8_forest_new_adapt(adapt_data.GetCurrentMesh(), _TestDecompressLightAMRPCPEncoding, 0, 0, static_cast<void*>(&adapt_data));
                cmc_debug_msg("Mesh adaptation finished");
                /* Store the decomrpessed mesh */
                adapt_data.SetCurrentMesh(decompressed_forest);
                cmc_debug_msg("New mesh has been stored");
                /* Finalize the decompression iteration */
                adapt_data.FinalizeDiffDecompression();
                cmc_debug_msg("Diff Decompression iteration: ", lvl, " has been finalized.");

                //if (lvl <= 6)
                //{
                    //compression_variables_.back().WriteDataToVTK_(1000 + lvl);
                //}
                //compression_variables_.back().WriteDataToFile("decompressed_cmc_sfc_data_" + std::to_string(lvl) + ".bin");
            }

            cmc_debug_msg("Decompression of variable: ", nc_var_name, " is finished.");
            /* Write data to file */
            compression_variables_.back().WriteDataToFile("decompressed_cmc_sfc_data.bin");
        }
    }
}
