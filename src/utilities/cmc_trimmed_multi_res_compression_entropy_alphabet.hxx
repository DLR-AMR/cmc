#ifndef CMC_TRIMMED_MULTI_RES_COMPRESSION_ENTROPY_ALPHABET_HXX
#define CMC_TRIMMED_MULTI_RES_COMPRESSION_ENTROPY_ALPHABET_HXX
...
namespace cmc::entropy_coding::arithmetic_coding
{
    ...
    constexpr uint32_t kTrimmedByteCompressionSignumBit = 0x40000000;

    constexpr inline bool
    CheckIfTrimmedCompressionSignumBitIsSet(const uint32_t symbol)
    {
        return ((symbol >> (sizeof(uint32_t) * bit_map::kCharBit - 2)) & uint32_t{1});
    }
    
    template <typename T>
    constexpr static size_t GetTrimmedMultiResAlphabetCount()
    {
    ...
    template <typename T>
    class TrimmedMultiResCompressionAlphabet : public IByteCompressionEntropyAlphabet
    {
    public:
        size_t GetAlphabetSize() const override {return GetTrimmedMultiResAlphabetCount<T>();};
    ...
    template <typename T>
    bool
    TrimmedMultiResCompressionAlphabet<T>::DoesSymbolExistInAlphabet(const uint32_t symbol)
    {
        if (symbol < kTrimmedByteCompressionSignumBit)
        {
        ...
        } else
        {
            ...
            if (symbol >= kTrimmedByteCompressionSignumBit && symbol <= kTrimmedByteCompressionSignumBit + sizeof(T) * bit_map::kCharBit)
            {
                return true;
            } else
            {
    ...
    template <typename T>
    uint32_t
    TrimmedMultiResCompressionAlphabet<T>::TransformSymbolToArrayIndex(const uint32_t symbol)
    {
        ...
        if (CheckIfCompressionSignumBitIsSet(symbol))
        {
            ...
            return (symbol - kTrimmedByteCompressionSignumBit) + offset;
    ...
    template <typename T>
    uint32_t
    TrimmedMultiResCompressionAlphabet<T>::RevertArrayIndexToSymbol(const uint32_t index)
    {
        ...
        } else
        {
            ...
            } else
            {
                ...
                return kTrimmedByteCompressionSignumBit + (index - offset);
            }
        }
    }
    ...
    template <typename T>
    std::vector<uint32_t>
    TrimmedMultiResCompressionAlphabet<T>::CommunicateSymbolFrequencies(const MPI_Comm comm)
    {
...
#endif /* !CMC_TRIMMED_MULTI_RES_COMPRESSION_ENTROPY_ALPHABET_HXX */
