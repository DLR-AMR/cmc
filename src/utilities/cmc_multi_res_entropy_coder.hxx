#ifndef CMC_MULTI_RES_ENTROPY_CODER_HXX
#define CMC_MULTI_RES_ENTROPY_CODER_HXX
...
template <typename T>
class MultiResEncoder : public Encoder<T>
{
public:
    MultiResEncoder()
...
template <typename T>
class MultiResDecoder : public Decoder
{
public:
    template <typename Iter> MultiResDecoder(Iter alphabet_begin)
    : Decoder(DecodeByteCompressionStaticFrequencyAlphabet<T>(alphabet_begin, std::make_unique<MultiResCompressionAlphabet<T>>())) {};
...
#endif /* !CMC_MULTI_RES_ENTROPY_CODER_HXX */
