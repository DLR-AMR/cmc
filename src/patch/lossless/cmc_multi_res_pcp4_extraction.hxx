#ifndef CMC_PATCH_LOSSLESS_MULTI_RES_PCP4_EXTRACTION_HXX
#define CMC_PATCH_LOSSLESS_MULTI_RES_PCP4_EXTRACTION_HXX
...
PerformExtraction(const std::array<T, kPackSize<DIM>> patch_data, const int num_values)
{
    ...
    if (num_values == 1) [[unlikely]]
    {
        ...
        zero_coding.entropy_symbols[0] = static_cast<SymbolType>(sizeof(T) * cmc::bits::kCharBit);
    ...
    {
        auto xor_diff = TransformToUInteger<T>(0);

        /* Check the predictor for all element values */
        for (int elem_idx{0}; elem_idx < num_values; ++elem_idx)
        {
            /* Compute the residual */
            const auto residual = cmc::bits::ComputeXORIntegerResidual<T>(predictors[pred_idx], predictors[elem_idx]);
            
            /* Store the residual */
            current_residuals.residuals[elem_idx] = residual;

            /* OR the residual to find the maximal applicable leading zero count */
            xor_diff |= residual;
        }

        /* Compute the cumulative leading zero count */
        const int pack_lzc = cmc::bits::GetLZC(xor_diff);

        /* If the predictor maximizes the LZC, we store it */
        if (current_lzc < pack_lzc)
        {
            current_lzc = pack_lzc;
            ...
            /* We store the length of the lzc in the first "entropy code" */
            patch_coding.entropy_symbols[0] = static_cast<SymbolType>(pack_lzc);
        }
    }
...
{
    static_assert(sizeof(T) >= 2);//The type needs to larger than the max LZC currently
    ...
    for (int32_t step_idx{1}; step_idx < num_encoding_steps; ++step_idx, ++enc_iter)
    {
        ...
        for (int32_t elem_idx{0}; elem_idx < num_elems; ++elem_idx)
        {
            ...
            /* Determine the applicable LZC from the packs max LZC */
            const uint8_t applicable_lzc = enc_data.entropy_symbols[0] > 15U ? uint8_t{15} : static_cast<uint8_t>(enc_data.entropy_symbols[0]);

            /* Store the applicable LZC fixed in four bit */
            lvl_data.AppendBits<uint8_t, int>(applicable_lzc, static_cast<int>(4), static_cast<int>(0));
            ...
            {
                /* Store the trimmed residual */
                //applicable_lzc < static_cast<uint8_t>(sizeof(T) * cmc::bits::kCharBit) hold always due to type restricitons (sizeof(T) >= 2)
                /* Append the significant reisdual bits */
                lvl_data.AppendBits(enc_data.residuals[val_idx], applicable_lzc, uint8_t{0});
...
CompressionVariable<T, DIM>::WriteCompressedData(const std::string& file_name)
{
    ...
    const uint64_t num_header_vals = 6 + this->levelwise_encoded_data_.size() + this->levelwise_encoded_data_.size() * DIM;
    ...
    const uint64_t global_byte_count = header_size + level_bytes;
...
#endif /* !CMC_PATCH_LOSSLESS_MULTI_RES_PCP4_EXTRACTION_HXX */
