#ifndef CMC_PATCH_FAST_MULTI_RES_DECOMPRESSION_WITH_CORRECTION_HXX
#define CMC_PATCH_FAST_MULTI_RES_DECOMPRESSION_WITH_CORRECTION_HXX
...
private:
    ...
    void PerformPredictionDecompression(const bool is_last_iteration, const T coarse_predictor,  const int32_t lon, const int32_t lat, const int32_t lev, const int32_t time, const std::array<int32_t, DIM>& next_lvl_dim_lengths);
    void PerformPredictionDecompression(const bool is_last_iteration, const T coarse_predictor,  const int32_t lon, const int32_t lat, const int32_t lev, const std::array<int32_t, DIM>& next_lvl_dim_lengths);
    void PerformPredictionDecompression(const bool is_last_iteration, const T coarse_predictor,  const int32_t lon, const int32_t lat, const std::array<int32_t, DIM>& next_lvl_dim_lengths);
    void PerformPredictionDecompression(const bool is_last_iteration, const T coarse_predictor,  const int32_t lon, const std::array<int32_t, DIM>& next_lvl_dim_lengths);
...
template <OneByteArithmeticType T>
inline T 
DecodeNextResidualAppliedValue(cmc::bits::StreamDecoder<SymbolType>& decoder, const OneByteResidualType u_coarse_predictor)
{
    /* Get the next entropy symbol */
    const SymbolType next_symbol = decoder.DecodeNextEntropySymbol(); 

    T decompressed_value{};

    /* Get the LZC from the symbol */
    const int lzc = GetLZCFromEntropySymbol(next_symbol);    
    if (lzc < static_cast<int>(sizeof(T) * cmc::bits::kCharBit) - 1) [[likely]]
    {
        ...
        const OneByteResidualType residual = decoder.GetNextBitSequence<OneByteResidualType>(residual_length) | (OneByteResidualType{1} << residual_length);

        /* We create the residual applied value */
        if (IsApproximationGreater(next_symbol))
        {
            /* Subtract the residual from the prediction */
            const OneByteResidualType value = cmc::bits::IntegerSubtraction(u_coarse_predictor, residual);

            decompressed_value = std::bit_cast<T>(value);
        } else
        {
            /* Add the residual to the prediction */
            const OneByteResidualType value = cmc::bits::IntegerAddition(u_coarse_predictor, residual); 
    ...
    } else
    {
        ...
        {
            /* Store the predictor */
            decompressed_value = std::bit_cast<T>(u_coarse_predictor);
        } else
        {
...
template <TwoByteArithmeticType T>
inline T 
DecodeNextResidualAppliedValue(cmc::bits::StreamDecoder<SymbolType>& decoder, const TwoByteResidualType u_coarse_predictor)
{
    /* Get the next entropy symbol */
    const SymbolType next_symbol = decoder.DecodeNextEntropySymbol(); 
    ...
    if (lzc < static_cast<int>(sizeof(T) * cmc::bits::kCharBit) - 1) [[likely]]
    {
        ...
        /* We obtain the residual and add the implicit one bit  */
        const TwoByteResidualType residual = decoder.GetNextBitSequence<TwoByteResidualType>(residual_length) | (TwoByteResidualType{1} << residual_length);

        /* We create the residual applied value */
        if (IsApproximationGreater(next_symbol))
        {
            /* Subtract the residual from the prediction */
            const TwoByteResidualType value = cmc::bits::IntegerSubtraction(u_coarse_predictor, residual);

            decompressed_value = std::bit_cast<T>(value);
        } else
        {
            /* Add the residual to the prediction */
            const TwoByteResidualType value = cmc::bits::IntegerAddition(u_coarse_predictor, residual); 
...
template <FourByteArithmeticType T>
inline T 
DecodeNextResidualAppliedValue(cmc::bits::StreamDecoder<SymbolType>& decoder, const FourByteResidualType u_coarse_predictor)
{
    /* Get the next entropy symbol */
    const SymbolType next_symbol = decoder.DecodeNextEntropySymbol(); 
    ...
    if (lzc < static_cast<int>(sizeof(T) * cmc::bits::kCharBit) - 1) [[likely]]
    {
        ...
        /* We obtain the residual and add the implicit one bit  */
        const FourByteResidualType residual = decoder.GetNextBitSequence<FourByteResidualType>(residual_length) | (FourByteResidualType{1} << residual_length);

        /* We create the residual applied value */
        if (IsApproximationGreater(next_symbol))
        {
            /* Subtract the residual from the prediction */
            const FourByteResidualType value = cmc::bits::IntegerSubtraction(u_coarse_predictor, residual);

            decompressed_value = std::bit_cast<T>(value);
        } else
        {
            /* Add the residual to the prediction */
            const FourByteResidualType value = cmc::bits::IntegerAddition(u_coarse_predictor, residual); 
...
template <EightByteArithmeticType T>
inline T 
DecodeNextResidualAppliedValue(cmc::bits::StreamDecoder<SymbolType>& decoder, const EightByteResidualType u_coarse_predictor)
{
    /* Get the next entropy symbol */
    const SymbolType next_symbol = decoder.DecodeNextEntropySymbol(); 
    ...
    if (lzc < static_cast<int>(sizeof(T) * cmc::bits::kCharBit) - 1) [[likely]]
    {
        ...
        /* We obtain the residual and add the implicit one bit  */
        const EightByteResidualType residual = decoder.GetNextBitSequence<EightByteResidualType>(residual_length) | (EightByteResidualType{1} << residual_length);

        /* We create the residual applied value */
        if (IsApproximationGreater(next_symbol))
        {
            /* Subtract the residual from the prediction */
            const EightByteResidualType value = cmc::bits::IntegerSubtraction(u_coarse_predictor, residual);

            decompressed_value = std::bit_cast<T>(value);
        } else
        {
            /* Add the residual to the prediction */
            const EightByteResidualType value = cmc::bits::IntegerAddition(u_coarse_predictor, residual); 
...
requires Dimension<DIM>
void
DecompressionVariable<T, DIM>::PerformPredictionDecompression(const bool is_last_iteration, const T coarse_predictor,  const int32_t lon, const int32_t lat, const int32_t lev, const int32_t time, const std::array<int32_t, DIM>& next_lvl_dim_lengths)
{
    static_assert(DIM == 4);
    ...
    /* Transform it to an unsigned type for residual computations */
    const auto u_coarse_predictor = TransformToUInteger<T>(coarse_predictor);

    /* Check which iteration type we have */
    if (is_last_iteration)
    {
        /* We have implicit values to compute */
        int num_elems{0};
        ...
        const int implicit_val_idx = num_elems > 1 ? num_elems - 1 : num_elems;
        int processed_elems{0};

        std::vector<T> previous_vals;
        previous_vals.reserve(implicit_val_idx);
            ...
            for (int32_t lev_idx = 0; lev_idx < kDIMReductionFactor; ++lev_idx)
            {
                for (int32_t lat_idx = 0; lat_idx < kDIMReductionFactor; ++lat_idx)
                {
                    for (int32_t lon_idx = 0; lon_idx < kDIMReductionFactor; ++lon_idx)
                    {
                        ...
                        {
                            if (processed_elems == implicit_val_idx) [[unlikely]]
                            {
                                /* Decompress the corresponding fine value */
                                const T fine_value = ComputeImplicitValue<T>(coarse_predictor, previous_vals);

                                /* Store the value for the next level data */
                                SetValue<T>(this->data_, fine_value, kDIMReductionFactor * time + time_idx, kDIMReductionFactor * lev + lev_idx,
                            ...
                            {
                                /* Decompress the corresponding fine value */
                                const T fine_value = DecodeNextResidualAppliedValue<T>(this->stream_decoder_, u_coarse_predictor);

                                previous_vals.push_back(fine_value);
    ...
    {
        /* There are no implciit values to decode, every value has it's entropy code */
        for (int32_t time_idx = 0; time_idx < kDIMReductionFactor; ++time_idx)
        {
            for (int32_t lev_idx = 0; lev_idx < kDIMReductionFactor; ++lev_idx)
            {
...
requires Dimension<DIM>
void
DecompressionVariable<T, DIM>::PerformPredictionDecompression(const bool is_last_iteration, const T coarse_predictor,  const int32_t lon, const int32_t lat, const int32_t lev, const std::array<int32_t, DIM>& next_lvl_dim_lengths)
{
    static_assert(DIM == 3);
                ...
                for (int32_t lon_idx = 0; lon_idx < kDIMReductionFactor; ++lon_idx)
                {
                    ...
                    } else
                    {
                        if (processed_elems == implicit_val_idx) [[unlikely]]
                        {
                            ...
                            /* Store the value for the next level data */
                            SetValue<T>(this->data_, fine_value, kDIMReductionFactor * lev + lev_idx,
...
requires Dimension<DIM>
void
DecompressionVariable<T, DIM>::PerformPredictionDecompression(const bool is_last_iteration, const T coarse_predictor,  const int32_t lon, const int32_t lat, const std::array<int32_t, DIM>& next_lvl_dim_lengths)
{
    static_assert(DIM == 2);
            ...
            for (int32_t lon_idx = 0; lon_idx < kDIMReductionFactor; ++lon_idx)
            {
                ...
                } else
                {
                    if (processed_elems == implicit_val_idx) [[unlikely]]
                    {
                        ...
                        /* Store the value for the next level data */
                        SetValue<T>(this->data_, fine_value,
...
requires Dimension<DIM>
void
DecompressionVariable<T, DIM>::PerformPredictionDecompression(const bool is_last_iteration, const T coarse_predictor,  const int32_t lon, const std::array<int32_t, DIM>& next_lvl_dim_lengths)
{
    static_assert(DIM == 1);
...
DecompressionVariable<T, DIM>::Decompress(DecompressionVariable<T, DIM>::kTag4D)
{
    ...
    cmc_debug_msg("Lossless patch-based decompression starts...");
    ...
    for (int32_t lvl_idx{1}; lvl_idx < this->num_decompression_levels_; ++lvl_idx, ++coarse_dim_lengths_iter)
    {
        ...
        /* Only in the last iteration we have to compute incomplete values */
        const bool is_last_iteration = (kTryToCorrectMeanFastCompression && lvl_idx == this->num_decompression_levels_ - 1);
            ...
            for (int32_t lev = 0; lev < coarse_dim_lengths[kLevID]; ++lev)
            {
                for (int32_t lat = 0; lat < coarse_dim_lengths[kLatID]; ++lat)
                {
                    for (int32_t lon = 0; lon < coarse_dim_lengths[kLonID]; ++lon)
                    {
                        ...
                        /* Get the predictor */
                        const T coarse_predictor = GetValue<T>(coarse_data, time, lev, lat, lon,
                                                               coarse_dim_lengths[kLonID], coarse_dim_lengths[kLatID], coarse_dim_lengths[kLevID], coarse_dim_lengths[kTimeID]);
                        
                        /* Perform the refinement prediction */
                        this->PerformPredictionDecompression(is_last_iteration, coarse_predictor, lon, lat, lev, time, next_lvl_dim_lengths);
                    }
                }
            }
        }
    ...
    cmc_debug_msg("The lossless decompression of is finished.");
}
        ...
        for (int32_t lev = 0; lev < coarse_dim_lengths[kLevID]; ++lev)
        {
            for (int32_t lat = 0; lat < coarse_dim_lengths[kLatID]; ++lat)
            {
                for (int32_t lon = 0; lon < coarse_dim_lengths[kLonID]; ++lon)
                {
                    ...
                    /* Get the predictor */
                    const T coarse_predictor = GetValue<T>(coarse_data, lev, lat, lon,
                                                           coarse_dim_lengths[kLonID], coarse_dim_lengths[kLatID], coarse_dim_lengths[kLevID]);
                    
                    /* Perform the refinement prediction */
                    this->PerformPredictionDecompression(is_last_iteration, coarse_predictor, lon, lat, lev, next_lvl_dim_lengths);
                }
            }
        }
    ...
    for (int32_t lvl_idx{1}; lvl_idx < this->num_decompression_levels_; ++lvl_idx, ++coarse_dim_lengths_iter)
    {
        ...
        for (int32_t lat = 0; lat < coarse_dim_lengths[kLatID]; ++lat)
        {
            for (int32_t lon = 0; lon < coarse_dim_lengths[kLonID]; ++lon)
            {
                ...
                /* Get the predictor */
                const T coarse_predictor = GetValue<T>(coarse_data, lat, lon,
                                                       coarse_dim_lengths[kLonID], coarse_dim_lengths[kLatID]);
                
                /* Perform the refinement prediction */
                this->PerformPredictionDecompression(is_last_iteration, coarse_predictor, lon, lat, next_lvl_dim_lengths);
            }
        }
...
DecompressionVariable<T, DIM>::Decompress(DecompressionVariable<T, DIM>::kTag1D)
{
    ...
    for (int32_t lvl_idx{1}; lvl_idx < this->num_decompression_levels_; ++lvl_idx, ++coarse_dim_lengths_iter)
    {
        ...
        for (int32_t lon = 0; lon < coarse_dim_lengths[kLonID]; ++lon)
        {
            ...
            /* Get the predictor */
            const T coarse_predictor = GetValue<T>(coarse_data, lon,
                                                   coarse_dim_lengths[kLonID]);
            
            /* Perform the refinement prediction */
            this->PerformPredictionDecompression(is_last_iteration, coarse_predictor, lon, next_lvl_dim_lengths);
        }
...
DecompressionVariable<T, DIM>::Decompress()
{
    ...
    if constexpr (DIM == 1)
    {
        cmc_global_msg("Lossless 1D Decompression");
        this->Decompress(DecompressionVariable<T, DIM>::tag1D);
    } else if constexpr (DIM == 2)
    {
        cmc_global_msg("Lossless 2D Decompression");
        this->Decompress(DecompressionVariable<T, DIM>::tag2D);
    } else if constexpr (DIM == 3)
    {
        cmc_global_msg("Lossless 3D Decompression");
        this->Decompress(DecompressionVariable<T, DIM>::tag3D);
    } else if constexpr (DIM == 4)
    {
        cmc_global_msg("Lossless 4D Decompression");
...
#endif /* !CMC_PATCH_FAST_MULTI_RES_DECOMPRESSION_WITH_CORRECTION_HXX */
