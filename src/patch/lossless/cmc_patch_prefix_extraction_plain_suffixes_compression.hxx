#ifndef CMC_PATCH_PREFIX_EXTRACTION_PLAIN_SUFFIXES_COMPRESSION_HXX
#define CMC_PATCH_PREFIX_EXTRACTION_PLAIN_SUFFIXES_COMPRESSION_HXX
...
namespace cmc::patch::lossless::prefix::plain_suffix
{
...
public:
    ...
    {
        return CompressionSchema::PatchPrefixExtractionPlainSuffixes;
    }
...
void
    ...
    std::unique_ptr<entropy_coding::IByteCompressionEntropyCoder> entropy_coder_{std::make_unique<cmc::entropy_coding::arithmetic_coding::PrefixEncoder<T>>()}; //!< The entropy coder to use in order to encode information
...
PatchCompressionVariable<T, Dim>::PerformExtraction(const std::vector<CompressionValue<T>>& patch)
{
    ...
    auto [is_prefix_found, prefix] = EvaluatePatchCommonPrefix<T>(patch);
    ...
    if (is_prefix_found)
    {
        ...
        /* Return the common prefix and the trimmed remaining values */
...
PatchCompressionVariable<T, Dim>::CollectSymbolFrequenciesForEntropyCoding(const std::vector<CompressionValue<T>>& level_byte_values) const
{
    ...
    for (auto val_iter = level_byte_values.begin(); val_iter != level_byte_values.end(); ++val_iter)
    {
        ...
        entropy_coder_->UpdateSymbolFrequency(pref_length);
    }
}
...
PatchCompressionVariable<T, Dim>::EncodeInteriorLevelData(const std::vector<CompressionValue<T>>& level_byte_values) const
{
    ...
    entropy_coder_->Reset(std::make_unique<cmc::entropy_coding::arithmetic_coding::PrefixCompressionAlphabet<T>>());
    ...
    for (auto val_iter = level_byte_values.begin(); val_iter != level_byte_values.end(); ++val_iter)
    {
        ...
        entropy_coder_->EncodeSymbol(static_cast<uint32_t>(pref_length));
    ...
    cmc::bit_map::BitMap local_encoded_prefix_length_stream = entropy_coder_->GetEncodedBitStream();
    ...
    /* The length of the encoded streams */
...
template<class T, size_t Dim>
std::pair<std::vector<uint8_t>, std::vector<uint8_t>>
PatchCompressionVariable<T, Dim>::EncodeLeafLevelData(const std::vector<CompressionValue<T>>& level_byte_values) const
{
...
#endif /* !CMC_PATCH_PREFIX_EXTRACTION_PLAIN_SUFFIXES_COMPRESSION_HXX */
