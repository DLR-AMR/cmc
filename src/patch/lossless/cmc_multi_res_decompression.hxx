#ifndef CMC_PATCH_LOSSLESS_MULTI_RES_DECOMPRESSION_HXX
#define CMC_PATCH_LOSSLESS_MULTI_RES_DECOMPRESSION_HXX
...
private:
    ...
    void PerformPredictionDecompression(const T coarse_predictor,  const int32_t lon, const int32_t lat, const int32_t lev, const int32_t time, const std::array<int32_t, DIM>& next_lvl_dim_lengths);
    void PerformPredictionDecompression(const T coarse_predictor,  const int32_t lon, const int32_t lat, const int32_t lev, const std::array<int32_t, DIM>& next_lvl_dim_lengths);
    void PerformPredictionDecompression(const T coarse_predictor,  const int32_t lon, const int32_t lat, const std::array<int32_t, DIM>& next_lvl_dim_lengths);
    void PerformPredictionDecompression(const T coarse_predictor,  const int32_t lon, const std::array<int32_t, DIM>& next_lvl_dim_lengths);
...
requires Dimension<DIM>
void
DecompressionVariable<T, DIM>::PerformPredictionDecompression(const T coarse_predictor,  const int32_t lon, const int32_t lat, const int32_t lev, const int32_t time, const std::array<int32_t, DIM>& next_lvl_dim_lengths)
{
    static_assert(DIM == 4);
...
requires Dimension<DIM>
void
DecompressionVariable<T, DIM>::PerformPredictionDecompression(const T coarse_predictor,  const int32_t lon, const int32_t lat, const int32_t lev, const std::array<int32_t, DIM>& next_lvl_dim_lengths)
{
    static_assert(DIM == 3);
...
requires Dimension<DIM>
void
DecompressionVariable<T, DIM>::PerformPredictionDecompression(const T coarse_predictor,  const int32_t lon, const int32_t lat, const std::array<int32_t, DIM>& next_lvl_dim_lengths)
{
    static_assert(DIM == 2);
...
requires Dimension<DIM>
void
DecompressionVariable<T, DIM>::PerformPredictionDecompression(const T coarse_predictor, const int32_t lon, const std::array<int32_t, DIM>& next_lvl_dim_lengths)
{
    static_assert(DIM == 1);
            ...
            for (int32_t lev = 0; lev < coarse_dim_lengths[kLevID]; ++lev)
            {
                for (int32_t lat = 0; lat < coarse_dim_lengths[kLatID]; ++lat)
                {
                    for (int32_t lon = 0; lon < coarse_dim_lengths[kLonID]; ++lon)
                    {
                        ...
                        this->PerformPredictionDecompression(coarse_predictor, lon, lat, lev, time, next_lvl_dim_lengths);
                    }
                }
            }
        }
        ...
        for (int32_t lev = 0; lev < coarse_dim_lengths[kLevID]; ++lev)
        {
            for (int32_t lat = 0; lat < coarse_dim_lengths[kLatID]; ++lat)
            {
                for (int32_t lon = 0; lon < coarse_dim_lengths[kLonID]; ++lon)
                {
                    ...
                    /* Perform the refinement prediction */
                    this->PerformPredictionDecompression(coarse_predictor, lon, lat, lev, next_lvl_dim_lengths);
                }
            }
        }
    ...
    for (int32_t lvl_idx{1}; lvl_idx < this->num_decompression_levels_; ++lvl_idx, ++coarse_dim_lengths_iter)
    {
        ...
        for (int32_t lat = 0; lat < coarse_dim_lengths[kLatID]; ++lat)
        {
            for (int32_t lon = 0; lon < coarse_dim_lengths[kLonID]; ++lon)
            {
                ...
                /* Perform the refinement prediction */
                this->PerformPredictionDecompression(coarse_predictor, lon, lat, next_lvl_dim_lengths);
            }
        }
...
DecompressionVariable<T, DIM>::Decompress(DecompressionVariable<T, DIM>::kTag1D)
{
    ...
    for (int32_t lvl_idx{1}; lvl_idx < this->num_decompression_levels_; ++lvl_idx, ++coarse_dim_lengths_iter)
    {
        ...
        for (int32_t lon = 0; lon < coarse_dim_lengths[kLonID]; ++lon)
        {
            ...
            /* Perform the refinement prediction */
            this->PerformPredictionDecompression(coarse_predictor, lon, next_lvl_dim_lengths);
        }
...
#endif /* !CMC_PATCH_LOSSLESS_MULTI_RES_DECOMPRESSION_HXX */
