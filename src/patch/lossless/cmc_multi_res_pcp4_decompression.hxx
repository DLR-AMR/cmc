#ifndef CMC_PATCH_LOSSLESS_MULTI_RES_PCP4_DECOMPRESSION_HXX
#define CMC_PATCH_LOSSLESS_MULTI_RES_PCP4_DECOMPRESSION_HXX
...
namespace cmc::patch::lossless::multi_res::pcp4
{
...
DecompressionVariable<T, DIM>::OpenSharedLevelDataWindow(const int level)
{
    ...
    const uint64_t file_offset = this->compression_info_.offset_start_encoding + var_level_offset;
...
template <TwoByteArithmeticType T>
inline T 
GetNextResidualAppliedValue(cmc::bits::StreamDecoder<SymbolType>& decoder, const TwoByteResidualType u_coarse_predictor, const int residual_length)
{
    /* Get the residual */
    const TwoByteResidualType residual = decoder.GetNextBitSequence<TwoByteResidualType>(residual_length);

    /* Apply the residual */
    const TwoByteResidualType value = u_coarse_predictor ^ residual;
...
template <FourByteArithmeticType T>
inline T 
GetNextResidualAppliedValue(cmc::bits::StreamDecoder<SymbolType>& decoder, const FourByteResidualType u_coarse_predictor, const int residual_length)
{
    /* Get the residual */
    const FourByteResidualType residual = decoder.GetNextBitSequence<FourByteResidualType>(residual_length);

    /* Apply the residual */
    const FourByteResidualType value = u_coarse_predictor ^ residual;
...
template <EightByteArithmeticType T>
inline T 
GetNextResidualAppliedValue(cmc::bits::StreamDecoder<SymbolType>& decoder, const EightByteResidualType u_coarse_predictor, const int residual_length)
{
    /* Get the residual */
    const EightByteResidualType residual = decoder.GetNextBitSequence<EightByteResidualType>(residual_length);

    /* Apply the residual */
    const EightByteResidualType value = u_coarse_predictor ^ residual;
...
DecompressionVariable<T, DIM>::PerformPredictionDecompression(const T coarse_predictor,  const int32_t lon, const int32_t lat, const int32_t lev, const int32_t time, const std::array<int32_t, DIM>& next_lvl_dim_lengths)
{
    ...
    /* Get the LZC for this pack of elements */
    const uint8_t lzc = this->stream_decoder_.GetNextBitSequence<uint8_t>(4);

    /* Compute the residual length */
    const int residual_length = sizeof(T) * cmc::bits::kCharBit - lzc;
            ...
            for (int32_t lat_idx = 0; lat_idx < kDIMReductionFactor; ++lat_idx)
            {
                for (int32_t lon_idx = 0; lon_idx < kDIMReductionFactor; ++lon_idx)
                {
                    ...
                    } else
                    {
                        ...
                        const T fine_value = GetNextResidualAppliedValue<T>(this->stream_decoder_, u_coarse_predictor, residual_length);
...
#endif /* !CMC_PATCH_LOSSLESS_MULTI_RES_PCP4_DECOMPRESSION_HXX */
