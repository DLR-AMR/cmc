...
/* Write the forest as well as the variables in a vtk-file (depending on the compression mode (if supplied) one file with all variables or one file per variable is written) */
void
cmc_t8_write_forest_all_vars(const CMC_T8_DATA& t8_data, const char* file_prefix)
{
    #ifdef CMC_WITH_T8CODE
    /* Arrays holding the ids of variables of the given data ordering */
    std::vector<int> var_ids_zcurve;
    var_ids_zcurve.reserve(t8_data.num_variables);
    std::vector<int> var_ids_linear;
    var_ids_linear.reserve(t8_data.num_variables);
    std::vector<int> var_ids_messy;
    var_ids_messy.reserve(t8_data.num_variables);

    int num_vars_undefined_data_scheme{0};
    /* Iterate over all variables and check if their data_scheme is compliant */
    for (int id{0}; id < t8_data.num_variables; ++id)
    {
        if (t8_data.var_data[id]->data_scheme == CMC_DATA_ORDERING_SCHEME::CMC_GEO_DATA_Z_CURVE)
        {
            var_ids_zcurve.push_back(id);
        } else if (t8_data.var_data[id]->data_scheme == CMC_DATA_ORDERING_SCHEME::CMC_GEO_DATA_LINEAR)
        {
            var_ids_linear.push_back(id);
        } else if(t8_data.var_data[id]->data_scheme == CMC_DATA_ORDERING_SCHEME::CMC_GEO_DATA_MESSY)
        {
            var_ids_messy.push_back(id);
        } 
        else if (t8_data.var_data[id]->data_scheme == CMC_DATA_ORDERING_SCHEME::CMC_GEO_DATA_SCHEME_UNDEFINED)
        {
            ++num_vars_undefined_data_scheme;
        }
    }
    ...
    /* If all data schemes are undefined, just write out the forest without any additional data */
    if (num_vars_undefined_data_scheme == t8_data.num_variables)
    {
        /* Check if an initial forest is supplied in t8_data */
        if (t8_data.initial_forest == nullptr)
        {
            cmc_err_msg("There is no initial_forest in t8_data.");
            return;
        }
        /* Write out the forest in a vtk file */
        int vtk_err{t8_forest_vtk_write_file (t8_data.initial_forest, file_prefix ,1,1,1,1,0,0,NULL)};
    ...
    /* In case there is some ordered data, write out the data with the forest itself */ 
    else
    {

        /* If all variables are defined on the same forest */
        if (t8_data.compression_mode == CMC_T8_COMPRESSION_MODE::ONE_FOR_ALL_2D ||
            t8_data.compression_mode == CMC_T8_COMPRESSION_MODE::ONE_FOR_ALL_3D)
        {
            /* Counts the IDs of the variables */
            int vtk_var_id{0};

            /* Get the amount of data per element (concerning the dimension of the data/the approach */
            const int num_data_vars{t8_data.num_variables - num_vars_undefined_data_scheme};
            
            /* Flag indicating if non-double variables had be to transformed */
            std::vector<bool> calculate_data;
            calculate_data.reserve(num_data_vars);
            ...
            /* Get the number of elements of the forest */
            const t8_locidx_t num_elems{t8_forest_get_local_num_elements(t8_data.t8_assets->forest)};
            /* Standard offset and scaling_factor */
            double scale_factor{1.0};
            double add_offset{0.0};
            ...
            for (auto iter{var_ids_zcurve.begin()}; iter != var_ids_zcurve.end(); ++iter)
            {
                scale_factor = cmc_get_universal_data<double>(t8_data.var_data[*iter]->scale_factor);
                add_offset = cmc_get_universal_data<double>(t8_data.var_data[*iter]->add_offset);

                calculate_data.push_back(!((t8_data.var_data[*iter]->data.get_member_data_type() == cmc_type::CMC_DOUBLE) && (t8_data.var_data[*iter]->offset_and_scaling_applied || (cmc_approx_cmp(add_offset, 0.0) && cmc_approx_cmp(scale_factor, 1.0)))));
                if (calculate_data.back())
                {
                    /* If the variable is not of type double */
                    /* Allocate an array for the current variable's element data */ 
                    ...
                    /* Check if the meta data of the variable holds offset and scaling attributes and if so, if they already have been applied */
                    if (t8_data.var_data[*iter]->offset_and_scaling_applied || (cmc_approx_cmp(add_offset, 0.0) && cmc_approx_cmp(scale_factor, 1.0)))
                    {
                        /* Assign data WITHOUT applying value offsets and scalings */
                        for (int elem_id{0}; elem_id < num_elems; ++elem_id)
                        {
                            if (elem_id % 400 == 0)
                            {
                                std::cout << "elem_id: " << elem_id << ", Value: " << t8_data.var_data[*iter]->data.get_data_of_member<float>(elem_id) << std::endl;
                            }
                            data_array[vtk_var_id][elem_id] = t8_data.var_data[*iter]->data.get_data_of_member<double>(elem_id);
                        }
                    } else
                    {
                        /* Assign data WITH applying value offsets and scalings */        
                        for (int elem_id{0}; elem_id < num_elems; ++elem_id)
                        {
                            data_array[vtk_var_id][elem_id] = add_offset + scale_factor * t8_data.var_data[*iter]->data.get_data_of_member<double>(elem_id);
                        }
                    }
                    /* Set the type of the data and pointer to the data */
                    snprintf(vtk_data[vtk_var_id].description, 50, "%s", (t8_data.var_data[*iter]->get_name()).c_str());
                    vtk_data[vtk_var_id].type = T8_VTK_SCALAR;
                    vtk_data[vtk_var_id].data = static_cast<double*>(data_array[vtk_var_id]);
                } else
                {
                    ...
                    snprintf(vtk_data[vtk_var_id].description, 50, "%s", (t8_data.var_data[*iter]->get_name()).c_str());
                    vtk_data[vtk_var_id].type = T8_VTK_SCALAR;
                    vtk_data[vtk_var_id].data = static_cast<double*>(t8_data.var_data[*iter]->data.get_initial_data_ptr_of_member());

                }
                /* Increment the vtk variable id */
                ++vtk_var_id;
            }

            // add variables with linear ordering (these have to be reorderd into z-cirve order but only temporarily)
            // TODO: The variables with differnt orderdings have to be taken into account too
            /* Write out the vtk file of the forest with the additional variable data */
            int vtk_err{t8_forest_vtk_write_file(t8_data.t8_assets->forest, file_prefix, 1, 1, 1, 1, 0, num_data_vars, vtk_data)};
            if (vtk_err == 0)
            {
                cmc_err_msg("An error occrued during the creation of the t8code-forest vtk file.");
            }
        ...
        /* If each variable is defined on its own forest */
            ...
            for (auto iter{var_ids_zcurve.begin()}; iter != var_ids_zcurve.end(); ++iter)
            {
                ...
                calculate_data = !((t8_data.var_data[*iter]->data.get_member_data_type() == cmc_type::CMC_DOUBLE) && (t8_data.var_data[*iter]->offset_and_scaling_applied || (cmc_approx_cmp(add_offset, 0.0) && cmc_approx_cmp(scale_factor, 1.0))));
                if (calculate_data)
                {

                /* Get the number of forest elements */
                num_elems = t8_forest_get_local_num_elements(t8_data.var_data[*iter]->t8_assets->forest);
                ...
                if (t8_data.var_data[*iter]->offset_and_scaling_applied || (cmc_approx_cmp(add_offset, 0.0) && cmc_approx_cmp(scale_factor, 1.0)))
                {
                    ...
                    for (int elem_id{0}; elem_id < num_elems; ++elem_id)
                    {
                        data_array[elem_id] = t8_data.var_data[*iter]->data.get_data_of_member<double>(elem_id);
                    }
                } else
                {
                    /* Assign data WITH applying value offsets and scalings */        
                    for (int elem_id{0}; elem_id < num_elems; ++elem_id)
                    {
                        data_array[elem_id] = add_offset + scale_factor * t8_data.var_data[*iter]->data.get_data_of_member<double>(elem_id);
                    }
                }

                /* Set the type of the data and pointer to the data */
                //snprintf(vtk_data[0].description, 50, "%s", (t8_data.var_data[*iter]->get_name()).c_str());
                vtk_data[0].type = T8_VTK_SCALAR;
                vtk_data[0].data = static_cast<double*>(data_array);
                ...
                } else
                {
                    ...
                    snprintf(vtk_data[0].description, 3, "%s", "O3");
                    vtk_data[0].type = T8_VTK_SCALAR;
                    vtk_data[0].data = static_cast<double*>(t8_data.var_data[*iter]->data.get_initial_data_ptr_of_member());
                }

                /* Assign a file prefix */
                //vtk_err = snprintf(_file_prefix, strlen(file_prefix) + 2 + strlen(t8_data.var_data[*iter]->get_name().c_str()), "%s_%s", file_prefix, t8_data.var_data[*iter]->get_name().c_str());
                //Quick fix for visualization
                vtk_err = snprintf(_file_prefix, strlen(file_prefix) + 10 + strlen(t8_data.var_data[*iter]->get_name().c_str()), "%s_%s%d", file_prefix,  "_O3_", *iter);
                #if 0
                if (*iter < 10)
                {
                vtk_err = snprintf(_file_prefix, strlen(file_prefix) + 10 + strlen(t8_data.var_data[*iter]->get_name().c_str()), "%s_%s%d%s", file_prefix, "_0", *iter, t8_data.var_data[*iter]->get_name().c_str());
                } else
                {
                    vtk_err = snprintf(_file_prefix, strlen(file_prefix) + 10 + strlen(t8_data.var_data[*iter]->get_name().c_str()), "%s_%s%d", file_prefix,  "_O3_", *iter,);
                }
                #endif
                /* Write out the variable data with the forest on which its data is defined */
                vtk_err = t8_forest_vtk_write_file(t8_data.var_data[*iter]->t8_assets->forest, _file_prefix, 1, 1, 1, 1, 0, 1, vtk_data);
                if (vtk_err == 0)
                {
                    cmc_err_msg("An error occrued during the creation of the t8code-forest vtk file.");
                }
            ...
            //Write out other variables which are defined in linear order (see above)
            //....
...
int
get_axis_id_in_ordering (const CMC_T8_VAR_DATA& var, const CMC_COORD_IDS _id)
{
    /* Return the position of the coordinate dimension */
    for (int i{0}; i < var.data_dim; ++i)
    {
        if (var.data_axis_ordering[i] == _id)
        {
            return i;
        }
    }
    /* If the id is not found in the ordering of the variable */
    return CMC_ERR;
}
