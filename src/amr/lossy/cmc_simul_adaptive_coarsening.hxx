#ifndef CMC_SIMUL_ADAPTIVE_COARSENING_HXX
#define CMC_SIMUL_ADAPTIVE_COARSENING_HXX

#include "amr/lossy/cmc_simul_ac_compression_variable.hxx"
...
{
public:
    CompressionVariable(t8_forest_t initial_mesh)
    ...
    void SetDataVariable(const std::string& name, const std::vector<T>& data) {this->SetVariable(name, data);}
};
...
{
    previous_number_of_elements_ = t8_forest_get_global_num_leaf_elements(base_variable_->GetAmrMesh().GetMesh());
}
...
{
    new_number_of_elements_ = t8_forest_get_global_num_leaf_elements(base_variable_->GetAmrMesh().GetMesh());
...
{
    //Nothing to be done here!
    cmc_err_msg("RepartitionData is currently not implemented.");
}
...
#endif /* !CMC_SIMUL_ADAPTIVE_COARSENING_HXX */
