#ifndef CMC_LOSSY_AMR_PAR_MULTI_RES_DECOMPRESSION_HXX
#define CMC_LOSSY_AMR_PAR_MULTI_RES_DECOMPRESSION_HXX
...
struct RefinementIterationData
{
    ...
    void PerformRefinement(const t8_eclass_t tree_class, const std::vector<T>& control_values, const std::vector<float>& permitted_abs_error);
    ...
    void PerformQuadPrediction(const std::vector<T>& control_values, const std::vector<float>& permitted_abs_error);
    void PerformHexPrediction(const std::vector<T>& control_values, const std::vector<float>& permitted_abs_error);
...
requires Dimension<DIM>
void
RefinementIterationData<T, DIM>::PerformRefinement(const t8_eclass_t tree_class, const std::vector<T>& control_values, const std::vector<float>& permitted_abs_error)
{
    if constexpr (DIM == 1)
    {
        /* 1D Elements */
    ...
    } else if constexpr (DIM == 2)
    {
        ...
        switch (tree_class)
        {
            case t8_eclass::T8_ECLASS_QUAD:
                this->PerformQuadPrediction(control_values, permitted_abs_error);
            break;
            case t8_eclass::T8_ECLASS_TRIANGLE:
                cmc_err_msg("The decompression for triangle elements is not yet implemented!");
            break;
            default:
                cmc_err_msg("The element type t8_eclass_t = ", static_cast<int>(tree_class), " is currently not supported!");
        }
    } else if constexpr (DIM == 2)
    {
        ...
        switch (tree_class)
        {
            case t8_eclass::T8_ECLASS_HEX:
                cmc_err_msg("The decompression for hex elements is not yet implemented!");
                this->PerformHexPrediction(control_values, permitted_abs_error);
            break;
            case t8_eclass::T8_ECLASS_TET:
                cmc_err_msg("The decompression for tetrahedral elements is not yet implemented!");
            break;
            case t8_eclass::T8_ECLASS_PRISM:
                cmc_err_msg("The decompression for prismatic elements is not yet implemented!");
            break;
            case t8_eclass::T8_ECLASS_PYRAMID:
                cmc_err_msg("The decompression for pyramidal elements is not yet implemented!");
            break;
            default:
                cmc_err_msg("The element type t8_eclass_t = ", static_cast<int>(tree_class), " is currently not supported!");
        }
...
requires Dimension<DIM>
void
RefinementIterationData<T, DIM>::PerformQuadPrediction(const std::vector<T>& control_vals, const std::vector<float>& permitted_abs_error)
{
    cmc_assert(control_vals.size() == cmc::par::lossy::rbf::util::kNumQuadControlPoints);
    
    /* Copy the control values */
    const std::array<T, cmc::par::lossy::rbf::util::kNumQuadControlPoints> control_values{control_vals[0], control_vals[1], control_vals[2], control_vals[3], control_vals[4]};

    /* Number of elements to construct */
    const int num_elements = 1 + cmc::par::lossy::rbf::util::kNumQuadPredictionPoints;

    /* Perform the quad prediction */
    const std::array<T, cmc::par::lossy::rbf::util::kNumQuadPredictionPoints> predictions = cmc::par::lossy::rbf::util::PerformQuadPrediction<T>(control_values);
    ...
    fam_predictions.push_back(control_values[0]);
    ...
    std::copy_n(fam_predictions.begin(), num_elements, std::back_inserter(this->fine_level_data));
}
                            ...
                            t8_element_t * elements[])
{
    ...
    if (adapt_data->WillNextElementBeRefined())
    {
        ...
        ts->element_new (tree_class, num_children, child_elements.data());

        /* Create the children and ask for their permitted absolute errors */
        ts->element_get_children(tree_class, elements[0], num_children, child_elements.data());
        ...
        adapt_data->PerformRefinement(tree_class, control_points, permitted_abs_errors);
...
DecompressionVariableMultiData<T, DIM, N>::GetDecompressedData()
{
    ...
    const int num_local_elements = t8_forest_get_local_num_leaf_elements(mesh);
    mesh_.SetNullMesh();
    std::vector<T> decompressed_data{};
    std::swap(decompressed_data, this->data_);
    /* Remove the ghost data */
    //decompressed_data.resize(num_local_elements);
    return std::make_pair(mesh, std::move(decompressed_data));
}

}
#endif /* !CMC_LOSSY_AMR_PAR_MULTI_RES_DECOMPRESSION_HXX */
