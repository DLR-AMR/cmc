#ifndef CMC_AMR_LOSSY_TRIXI_PAR_MULTI_RES_EXTRACTION_HXX
#define CMC_AMR_LOSSY_TRIXI_PAR_MULTI_RES_EXTRACTION_HXX
...
#include "amr/lossy/trixi/cmc_par_multi_res_util.hxx"
...
public:
    ...
    CompressionVariableMultiData(std::string name, t8_forest_t forest, const std::span<T> data, const std::vector<ErrorDomain>& error_domains, const std::vector<cmc::par::lossy::idw::util::Coordinate<DIM>>& reference_coordinates = std::vector<cmc::par::lossy::idw::util::Coordinate<DIM>>(1))
    : name_(name), mesh_(forest), init_data_(data), error_domains_(error_domains), reference_coordinates_(reference_coordinates) {
        if (integral_pow<static_cast<uint32_t>(DIM)>(N) != reference_coordinates_.size())
        {
            cmc_err_msg("The number of reference coordinates does not match the expected number of data points per element.");
        }
...
private:
    ...
    void PerformIntraElementCompression(const std::vector<T>& intra_elem_prediction_data, const std::vector<T>& init_data, const SizeType partition_bound);
    void EncodeData();
    ...
    void CollectIntraElementPredictionPyramid(const int num_local_mesh_elems);
    ...
    /* The permitted error domains */
    const std::vector<ErrorDomain> error_domains_;

    /* The reference coordinates of the DG coefficients per element */
    std::vector<cmc::par::lossy::idw::util::Coordinate<DIM>> reference_coordinates_;
    ...
    /* Counter of the applied compression steps */
    ...
    /* Information about the compression partitioning of the single steps */
    std::vector<PartitionInfo> level_partitioning_info_;

    /* Members filled by the intra element compression */
    std::vector<IntraElemEncodingData<T, DIM, N>> intra_elem_encodings_;
    ...
    std::vector<uint64_t> intra_elem_encoded_data_;

    /* Flag whether the data is already partitioned for coarsening */
...
CompressionIterationData<T, DIM>::PerformRefinementPrediction(const std::span<T> initial_values, const std::vector<cmc::par::lossy::idw::util::PointData<T, DIM>>& control_values,  const std::vector<cmc::par::lossy::idw::util::Coordinate<DIM>>& eval_coords, const std::vector<float>& permitted_abs_error)
{
    ...
    fam_predictions.reserve(eval_coords.size());
    ...
    for (int elem_idx{0}; elem_idx < num_elems; ++elem_idx)
    {
        ...
        } else if (residual <= kResidualMaxDeviationFactor * permitted_abs_error[elem_idx])
        {
            ...
            const SymbolType bin = static_cast<SymbolType>(std::floor(residual / (2.0 * permitted_abs_error[elem_idx]) + 0.5));
                          ...
                          t8_element_t * elements[])
{
    ...
    if (adapt_data->WillNextElementBeRefined())
    {
        ...
        for (int face_idx{0}; face_idx < num_faces; ++face_idx)
        {
            ...
            {
                /* Convert to ghost local tree id (it is either a local or a ghost tree) */
...
CompressionVariableMultiData<T, DIM, N>::RepartitionForCompressionIteration(t8_forest_t mesh, std::vector<T>& data, const SizeType previous_bound)
{
    ...
    /* Get the number of ghost elements of the mesh */
...
requires Dimension<DIM>
inline void
CompressionVariableMultiData<T, DIM, N>::CollectIntraElementPredictionPyramid(const int num_local_mesh_elems)
{
    /* In case more than a single value is given on each element */
    if constexpr (N > 1)
    {
        cmc_assert(this->init_data_.size() == num_local_mesh_elems * integral_pow<DIM>(N));
        ...
        /* Only for N > 2, there is an additional prediction step for the intra element coding */
        if constexpr (N > 2)
        {
            /* In the next step, we store the prediction coefficients in the pyramid */
            constexpr int num_control_points = GetNumIntraElemControlValues<DIM>();

            std::vector<T> first_lvl_data;
            first_lvl_data.reserve(num_local_mesh_elems * num_control_points);

            /* Get the array indices to extract the control values */
            constexpr std::array<int, kNumIntraElemControlValues<DIM>> control_indices = GetControlValueIndices<DIM, N>();

            int32_t data_offset{0};

            for (int elem_idx{0}; elem_idx < num_local_mesh_elems; ++elem_idx)
            {
                int32_t intra_elem_control_value_offset{0};

                /* Add the control values from this element to the prediction data */
                for (int idx{0}; idx < kNumIntraElemControlValues<DIM>; ++idx)
                {
                    first_lvl_data.push_back(this->init_data_[data_offset + control_indices[idx]]);
                }

                /* Get the offset to the next element */
                data_offset += integral_pow<DIM>(N);
            }

            /* Store the data in the prediction pyramid */
            this->data_pyramid_.push_back(std::move(first_lvl_data));
        }
        ...
        /* Emplace the single-data-per-element-vector to the data pyramid */
        this->data_pyramid_.emplace_back();
        std::vector<T>& second_lvl_data = this->data_pyramid_.back();
        second_lvl_data.reserve(num_local_mesh_elems);

        /* Get a refrence to the previous element values */
        const std::vector<T>& first_lvl_vals = *(std::prev(this->data_pyramid_.end(), 2));

        /* Extract a single value per element */
        for (int elem_idx{0}; elem_idx < num_local_mesh_elems; ++elem_idx)
        {
            /* Compute the element's mean value */
            const T mean_value = static_cast<T>(
                static_cast<double>(std::reduce(first_lvl_vals.data() + data_offset, first_lvl_vals.data() + data_offset + num_control_points)) / static_cast<double>(num_control_points)
            ); 

            /* Store the mean value in the data pyramid */
            second_lvl_data.push_back(mean_value);

            /* Move the offset to the next element's control values */
            data_offset += num_control_points;
        }

        /* Store the offsets of the mesh (this is needed in order to perform the prediction later on correctly) */
        const SizeType mesh_offset = static_cast<SizeType>(t8_forest_get_first_local_leaf_element_id(this->mesh_.GetMesh()));
        this->level_partition_offset_.push_back(mesh_offset);
    } else
    {
        /* Store the initial data in the data pyramid */
...
CompressionVariableMultiData<T, DIM, N>::Compress()
{
    ...
    this->error_mesh_ = std::make_unique<ErrorMesh>(this->mesh_.GetMesh(), this->error_domains_, this->init_data_, kGetNumPointsPerElement<DIM, N>());

    /* Get the number of initial local elements */
    const int num_local_elements = t8_forest_get_local_num_leaf_elements(this->mesh_.GetMesh());
    
    /* Perfom Intra Element Coarse-Predictor Extraction */
    this->CollectIntraElementPredictionPyramid(num_local_elements);
    ...
    if constexpr (N > 1)
    {
        ...
        /* Perform the intra element compression */
        this->PerformIntraElementCompression(*coarse_data_iter, *(std::next(coarse_data_iter)), *level_partition_iter);
    }
...
template<ArithmeticType T, int32_t DIM, int32_t N>
requires Dimension<DIM>
IntraElemEncodingData<T, DIM, N>
ComputeIntraElementEncoding(const T single_val, const std::span<const T>& control_values, const std::span<const T>& init_data, const float permitted_abs_error, const std::vector<cmc::par::lossy::idw::util::Coordinate<DIM>>& reference_coordinates)
{
    cmc_assert(kNumIntraElemControlValues<DIM> == control_values.size());

    /* Allocate memory for the intra element compression computations */
    IntraElemEncodingData<T, DIM, N> intra_elem_coding;

    /* First, we need to predict the element's control values with a constant interpolation */
    constexpr int num_control_values = kNumIntraElemControlValues<DIM>;

    /* Allocate the predicted control values (needed for the prediciton of the init data) */
    std::array<T, num_control_values> control_values_predictions;

    /* Perform the refinement prediciton of the control values */
    for (int control_elem_idx{0}; control_elem_idx < num_control_values; ++control_elem_idx)
    {
        /* Compute the residual between the control value and constant interpolation value */
        const T residual = GetAbsResidual<T>(single_val, control_values[control_elem_idx]);

        /* Determine whether the prediction is in line with the permitted error */
        if (residual <= permitted_abs_error)
        {
            /* If the residual is within the permitted error bound */
            intra_elem_coding.control_quantization_bins[control_elem_idx] = kPredictionWithinBound;

            /* We store the prediction */
            control_values_predictions[control_elem_idx] = single_val;

        } else if (residual <= kResidualMaxDeviationFactor * permitted_abs_error)
        {
            /* If the residual is within the permitted error interval such that the error can be met by quantization */
            const SymbolType bin = static_cast<SymbolType>(std::floor(residual / (2.0 * permitted_abs_error) + 0.5));

            /* Check in which direction the quantization goes */
            const bool is_prediction_greater = (single_val >= control_values[control_elem_idx]);

            /* Create the entropy symbol from the information above */
            const SymbolType entropy_symbol = CreateEntropySymbolFromQuantizationBin(bin, is_prediction_greater); 

            /* In this case, we do not need to store anything apart the quantization bin */
            intra_elem_coding.control_quantization_bins[control_elem_idx] = entropy_symbol;

            /* De-Quantize the value */
            const T decompressed_value = single_val + (is_prediction_greater ? -2.0 : +2.0) * permitted_abs_error * bin;

            /* We store the de-quantized value */
            control_values_predictions[control_elem_idx] = decompressed_value;

        } else
        {
            /* In this case, the value is unpredictable and we store it, as it is */
            /* We flag the value as unpredictable */
            intra_elem_coding.control_quantization_bins[control_elem_idx] = kFlagUnpredictable;
            
            /* And we store the actual value */
            intra_elem_coding.unpredictable_values.push_back(control_values[control_elem_idx]);
            control_values_predictions[control_elem_idx] = control_values[control_elem_idx];
        }
    }

    /** After the control values have been predicted, we are able to predict the initial element values based on the reference coordinates **/

    /* We obtain the the indices of the control values regarding the ordering respective to the reference coordinates */
    std::array<int, kNumIntraElemControlValues<DIM>> control_value_indices = GetControlValueIndices<DIM, N>();

    /* Create the prediction control values */
    std::vector<cmc::par::lossy::idw::util::PointData<T, DIM>> prediction_control_values(kNumIntraElemControlValues<DIM>);
    for (int cidx{0}; cidx < kNumIntraElemControlValues<DIM>; ++cidx)
    {
        prediction_control_values[cidx].data = control_values_predictions[cidx];
        prediction_control_values[cidx].coordinates = reference_coordinates[control_value_indices[cidx]];
    }

    /* Get the reference coordinates of the to-be-predicted data points */
    constexpr std::array<int, kNumInitDataPredictions<DIM, N>> prediction_reference_coord_ids = GetPredictionReferenceCoordinates<DIM, N>();

    /* We iterate over the prediction element values and try to predict them with an IDW interpolation */
    for (int data_idx{0}; data_idx < kNumInitDataPredictions<DIM, N>; ++data_idx)
    {
        /* Perform the prediction */
        const T prediction = cmc::par::lossy::idw::util::ComputePrediction<T>(prediction_control_values, reference_coordinates[prediction_reference_coord_ids[data_idx]]);

        /* Compute the residual between the initial value and interpolated prediction value */
        const T residual = GetAbsResidual<T>(prediction, init_data[prediction_reference_coord_ids[data_idx]]);

        /* Determine whether the prediction is in line with the permitted error */
        if (residual <= permitted_abs_error)
        {
            /* If the residual is within the permitted error bound */
            intra_elem_coding.init_quantization_bins[data_idx] = kPredictionWithinBound;
        } else if (residual <= kResidualMaxDeviationFactor * permitted_abs_error)
        {
            /* If the residual is within the permitted error interval such that the error can be met by quantization */
            const SymbolType bin = static_cast<SymbolType>(std::floor(residual / (2.0 * permitted_abs_error) + 0.5));
            ...
            /* Check in which direction the quantization goes */
            const bool is_prediction_greater = (prediction >= init_data[prediction_reference_coord_ids[data_idx]]);

            /* Create the entropy symbol from the information above */
            const SymbolType entropy_symbol = CreateEntropySymbolFromQuantizationBin(bin, is_prediction_greater); 

            /* In this case, we do not need to store anything apart the quantization bin */
            intra_elem_coding.init_quantization_bins[data_idx] = entropy_symbol;
        } else
        {
            /* In this case, the value is unpredictable and we store it, as it is */
            /* We flag the value as unpredictable */
            intra_elem_coding.init_quantization_bins[data_idx] = kFlagUnpredictable;
            
            /* And we store the actual value */
            intra_elem_coding.unpredictable_values.push_back(init_data[prediction_reference_coord_ids[data_idx]]);
        }
    }

    return intra_elem_coding;
}

template<ArithmeticType T, int32_t DIM, int32_t N>
requires Dimension<DIM>
void
CompressionVariableMultiData<T, DIM, N>::PerformIntraElementCompression(const std::vector<T>& intra_elem_prediction_data, const std::vector<T>& init_data, const SizeType partition_bound)
{
    cmc_global_msg("intra_elem_prediction_data.size(): ", intra_elem_prediction_data.size(), ", init_data.size(): ", init_data.size(), ", partition_bound: ", partition_bound);
    /* Get the current mesh with one value per element */
    t8_forest_t mesh = this->mesh_.GetMesh();

    /* Get the number of current mesh elements */
    const t8_locidx_t current_local_elems = t8_forest_get_local_num_leaf_elements(mesh);

    cmc_assert(current_local_elems == static_cast<t8_locidx_t>(this->data_.size()));
    ...
    /* Set the forest for partitioning */
    t8_forest_set_partition (init_mesh, mesh, 0);

    /* Set the partition bound explicitly */
    t8_forest_set_partition_offset (init_mesh, static_cast<t8_gloidx_t>(partition_bound));

    /* Commit the forest, this step will perform the partitioning and ghost layer creation. */
    t8_forest_commit (init_mesh);

    /* Get the number of local elements after partitioning */
    const t8_locidx_t num_local_elements = t8_forest_get_local_num_leaf_elements(init_mesh);
    
    /* Create an sc_array_t wrapper of the variable's local element data */
    sc_array_t* in_data = sc_array_new_data (static_cast<void*>(this->data_.data()), sizeof(T), current_local_elems);

    /* Allocate an output vector for the partitioned data */
    std::vector<T> partitioned_data(num_local_elements);

    /* Create a wrapper for the freshly allocated partitioned data */
    sc_array_t* out_data = sc_array_new_data (static_cast<void*>(partitioned_data.data()), sizeof(T), num_local_elements);

    /* Partition the variables local element data */
    t8_forest_partition_data(mesh, init_mesh, in_data, out_data);
    ...
    /* Update the current data to the correctly partitioned data */
    this->data_ = std::move(partitioned_data);

    int elem_offset{0};
    int control_value_offset{0};
    int init_value_offset{0};

    this->intra_elem_encodings_ = std::vector<IntraElemEncodingData<T, DIM, N>>();
    this->intra_elem_encodings_.reserve(num_local_elements);

    /* Get the forest's scheme */
    const t8_scheme_c* scheme = t8_forest_get_scheme(init_mesh);

    /* Get the number of local trees in the mesh */
    const t8_locidx_t num_local_trees = t8_forest_get_num_local_trees(init_mesh);

    /* Iterate over all elements in all trees */
    for (int tree_idx{0}; tree_idx < num_local_trees; ++tree_idx)
    {
        /* Get the class of the tree */
        const t8_eclass_t tree_class = t8_forest_get_tree_class (init_mesh, tree_idx);

        /* Get the number of elements in the tree */
        const t8_locidx_t  num_elements_in_tree = t8_forest_get_tree_num_leaf_elements (init_mesh, tree_idx);

        /* Get the global tree id */
        const int global_tree_id = t8_forest_global_tree_id (init_mesh, tree_idx);

        /* Iterate over all tree-local elements and encode them */
        for (int elem_idx = 0; elem_idx < num_elements_in_tree; ++elem_idx)
        {
            /* Get the current element */
            const t8_element_t* element = t8_forest_get_leaf_element_in_tree (init_mesh, tree_idx, elem_idx);

            /* Get the permmitted absolute error for this element */
            const float permitted_abs_error = this->error_mesh_->GetPermittedAbsError(scheme, global_tree_id, tree_class, element);

            /* Get the element's corresponding data */
            const T single_val = this->data_[elem_offset];
            const std::span<const T> control_values(intra_elem_prediction_data.data() + control_value_offset, kNumIntraElemControlValues<DIM>);
            const std::span<const T> init_values(init_data.data() + init_value_offset, kGetNumPointsPerElement<DIM, N>());

            /* Compute the encoding data */
            this->intra_elem_encodings_.emplace_back(ComputeIntraElementEncoding<T, DIM, N>(single_val, control_values, init_values, permitted_abs_error, this->reference_coordinates_));

            /* Update the offsets */
            ++elem_offset;
            control_value_offset += kNumIntraElemControlValues<DIM>;
            init_value_offset += kGetNumPointsPerElement<DIM, N>();
        }
    }
...
ExchangeIntraElementEntropySymbols(const std::vector<IntraElemEncodingData<T, DIM, N>>& intra_elem_encodings, const MPI_Comm comm)
{
    if constexpr (N > 1)
    {
        ...
        constexpr int kNumControlPoints = kNumIntraElemControlValues<DIM>;
        constexpr int kNumInitPredictionPoints = kNumInitDataPredictions<DIM, N>;

        /* Iterate through all entropy codes and accumulate their frequencies */
        for (size_t elem_idx{0}; elem_idx < intra_elem_encodings.size(); ++elem_idx)
        {
            /* Iterate first over the control points */
            for (int cidx{0}; cidx < kNumControlPoints; ++cidx)
            {
                /* Convert the symbol to the corresponding array index */
                const int array_idx = MapEntropySymbolToArrayIndex<T>(intra_elem_encodings[elem_idx].control_quantization_bins[cidx]);
                /* Update the frequency */
                ++entropy_symbol_frequencies[array_idx];
            }

            /* Iterate afterwards over the init data prediction points */
            for (int iidx{0}; iidx < kNumInitPredictionPoints; ++iidx)
            {
                /* Convert the symbol to the corresponding array index */
                const int array_idx = MapEntropySymbolToArrayIndex<T>(intra_elem_encodings[elem_idx].init_quantization_bins[iidx]);
                /* Update the frequency */
                ++entropy_symbol_frequencies[array_idx];
            }
        }
    ...
    {
        /* In case there is no intra-element compression, we return an empty vector */
        return std::vector<cmc::entropy_coding::huffman::EntropySymbol<SymbolType>>();
    }
}
...
CompressionVariableMultiData<T, DIM, N>::EncodeData()
{
    ...
    /* Collect and exchange the intra-element entropy symbols */
    [[maybe_unused]] const std::vector<cmc::entropy_coding::huffman::EntropySymbol<SymbolType>> intra_elem_entropy_symbols = ExchangeIntraElementEntropySymbols<T, DIM, N>(this->intra_elem_encodings_, this->comm_);

    /* Collect and exchange all mesh-level entropy symbols */
    ...
    /* Create a Huffman encoder for the mesh levels  */
    cmc::entropy_coding::huffman::HuffmanCoder<SymbolType> entropy_coder(entropy_symbols);

    /* Store the serialized Huffman coder of the mesh levels */
    ...
    if constexpr (N > 1)
    {
        /** In case there is an intra element compression step **/

        /* At last, we do encode the intra element coding */
        const std::vector<IntraElemEncodingData<T, DIM, N>>& intra_elem_codings = this->intra_elem_encodings_;

        /* Allocate an encoding vector */
        cmc::bits::vector intra_elem_data;
        intra_elem_data.Reserve((intra_elem_codings.size() * sizeof(IntraElemEncodingData<T, DIM, N>) * cmc::bits::kCharBit) / 2);

        /* Create the intra element Huffman coder */
        cmc::entropy_coding::huffman::HuffmanCoder<SymbolType> intra_elem_entropy_coder(intra_elem_entropy_symbols);

        /* Store the serialized Huffman coder of the intra element coding */
        this->serialized_intra_element_entropy_dictionary_ = intra_elem_entropy_coder.SerializeHuffmanCodesBEPadded();

        constexpr int kNumControlPoints = kNumIntraElemControlValues<DIM>;
        constexpr int kNumInitPredictionPoints = kNumInitDataPredictions<DIM, N>;

        /* Iterate over the intra element encodings and serialize them */
        for (size_t elem_idx{0}; elem_idx < intra_elem_codings.size(); ++elem_idx)
        {
            int unpredictable_idx{0};

            /* Iterate over all control values */
            for (int cidx{0}; cidx < kNumControlPoints; ++cidx)
            {
                /* Encode the entropy symbol */
                const cmc::entropy_coding::huffman::HuffmanCode code = intra_elem_entropy_coder.EncodeSymbol(intra_elem_codings[elem_idx].control_quantization_bins[cidx]);

                /* Serialize the encoded entropy symbol */
                intra_elem_data.AppendBits(code.code_word, static_cast<int>(sizeof(cmc::entropy_coding::huffman::HuffmanCodeWord) * cmc::bits::kCharBit - code.code_length), 0);
            
                /* If the data is not predicatble, we need to store the actual value */
                if (intra_elem_codings[elem_idx].control_quantization_bins[cidx] == kFlagUnpredictable) [[unlikely]]
                {
                    /* Append the not-predictable value fully */
                    intra_elem_data.AppendBits(TransformToUInteger<T>(intra_elem_codings[elem_idx].unpredictable_values[unpredictable_idx]), 0, 0);
                    ++unpredictable_idx;
                }
            }

            /* Iterate over all prediction data points */
            for (int iidx{0}; iidx < kNumInitPredictionPoints; ++iidx)
            {
                /* Encode the entropy symbol */
                const cmc::entropy_coding::huffman::HuffmanCode code = intra_elem_entropy_coder.EncodeSymbol(intra_elem_codings[elem_idx].init_quantization_bins[iidx]);

                /* Serialize the encoded entropy symbol */
                intra_elem_data.AppendBits(code.code_word, static_cast<int>(sizeof(cmc::entropy_coding::huffman::HuffmanCodeWord) * cmc::bits::kCharBit - code.code_length), 0);
            
                /* If the data is not predicatble, we need to store the actual value */
                if (intra_elem_codings[elem_idx].init_quantization_bins[iidx] == kFlagUnpredictable) [[unlikely]]
                {
                    /* Append the not-predictable value fully */
                    intra_elem_data.AppendBits(TransformToUInteger<T>(intra_elem_codings[elem_idx].unpredictable_values[unpredictable_idx]), 0, 0);
                    ++unpredictable_idx;
        ...
        /* At the end of the local encoding of the level, we append the process-end symbol */
        const cmc::entropy_coding::huffman::HuffmanCode process_lvl_end_code = intra_elem_entropy_coder.EncodeSymbol(kProcessEndSymbol);

        /* Serialize the encoded process end symbol */
        intra_elem_data.AppendBits(process_lvl_end_code.code_word, static_cast<int>(sizeof(cmc::entropy_coding::huffman::HuffmanCodeWord) * cmc::bits::kCharBit - process_lvl_end_code.code_length), 0);

        /* We store this level's encoding in the variable's buffer */
        this->intra_elem_encoded_data_ = intra_elem_data.GetSerializedByteStreamBE();

        /* We store the encoding length in the partition info */
        this->level_partitioning_info_.back().num_bytes_encoding = this->intra_elem_encoded_data_.size() * sizeof(uint64_t);
    }
}
...
GetVarHeaderSize(const int mesh_compression_levels, const bool has_intra_element_compression)
{
    ...
    return (sizeof(SizeType) //Global Number of Bytes (this value included)
            ...
            + sizeof(SizeType) //Num Data per dimension
            ...
            + (has_intra_element_compression ? sizeof(SizeType) * ((DIM * kGetNumPointsPerElement<DIM, N>()) / 2 + ((DIM * kGetNumPointsPerElement<DIM, N>()) % 2 != 0 ? 1 : 0)) : 0) //Num bytes reference coordinates
           );
}
                                                                      ...
                                                                      const int mesh_compression_levels, const std::vector<SizeType>& global_bytes_per_level)
{
    ...
    [[maybe_unused]] const SizeType num_bytes_var_header = GetVarHeaderSize<DIM, N>(num_global_mesh_encoding_steps, this->HasIntraElementCompression());
    ...
    /* Store the number of data per element per dimension */
    ...
    const SizeType num_intra_compression_levels = (this->HasIntraElementCompression() ? 1 : 0);
    stream.push_back(cmc::bits::ConvertToBigEndian<SizeType>(num_intra_compression_levels));
    ...
    /* At last, we store the reference coordinates if there is an intra-element compression */
    if constexpr (N > 1)
    {
        static_assert(sizeof(uint32_t) == sizeof(float));

        uint64_t packed_data{0};
        int front_back_indicator{0};
        /* Iterate over all reference coordinates */
        for (const cmc::par::lossy::idw::util::Coordinate<DIM>& coords : this->reference_coordinates_)
        {
            /* Iterate over all dimensional coordinates */
            for (auto dim_iter = coords.coordinates.begin(); dim_iter != coords.coordinates.end(); ++dim_iter)
            {
                if (front_back_indicator % 2 == 0)
                {
                    /* Push it to the front */
                    packed_data = (static_cast<uint64_t>(std::bit_cast<uint32_t>(*dim_iter)) << 32);
                } else
                {
                    /* Push it to the back and store it */
                    packed_data |= static_cast<uint64_t>(std::bit_cast<uint32_t>(*dim_iter));
                    stream.push_back(cmc::bits::ConvertToBigEndian<SizeType>(packed_data));
                }
                /* Update the indicator for the next iteration */
                ++front_back_indicator;
            }
        }
        /* Potentially, push the last incomplete uint64_t to the stream */
        if (front_back_indicator % 2 == 1)
        {
            /* Store the incomplete value */
            stream.push_back(cmc::bits::ConvertToBigEndian<SizeType>(packed_data));
        }
    }
}
...
CompressionVariableMultiData<T, DIM, N>::GenerateOuputStreams()
{
    ...
    const uint64_t start_encoding_offset_ = GetVarHeaderSize<DIM, N>(num_global_mesh_encoding_steps, this->HasIntraElementCompression())
    ...
    if (this->HasIntraElementCompression())
    {
        ...
        for (int rank_id{0}; rank_id < this->comm_size_; ++rank_id)
        {
            ...
            {
                /* If we match the turn of the serialized ouput, we store the current offset */
                this->output_streams_.emplace_back(static_cast<MPI_Offset>(current_byte_offset), std::move(this->intra_elem_encoded_data_));
                ...
                /* Overwrite the vector from which the data has been moved */
                this->intra_elem_encoded_data_ = std::vector<uint64_t>();
    ...
    if (this->comm_rank_ == kRootRank)
    {
        ...
        /* Append the Huffman Codes from the intra-element encoding step */
        ...
        /* Check the anticipated offset for correctness */
        cmc_debug_msg("Computed Byte-Start-Offset for Level-wise encodings: ", start_encoding_offset_, "; Actual Root-Rank File-Header-Byte-Length: ", root_rank_start_stream.size() * sizeof(uint64_t));
...
CompressionVariableMultiData<T, DIM, N>::WriteCompressedData(const std::string& file_name)
{
    ...
    cmc_debug_msg(this->comm_, "The file has been pre-allocated.");

    /* Allocate a MPI status object */
    MPI_Status status;

    /* Iterate over all data streams and place them in the file */
    for (const auto& stream : this->output_streams_)
    {
        cmc_debug_msg("Rank ", this->comm_rank_, " writes ", stream.data_stream.size() * sizeof(uint64_t), " bytes at the (byte-)offset ", stream.offset, " into the compressed file.");
...
#endif /* !CMC_AMR_LOSSY_TRIXI_PAR_MULTI_RES_EXTRACTION_HXX */
