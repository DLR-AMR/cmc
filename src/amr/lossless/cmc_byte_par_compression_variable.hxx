#ifndef LOSSLESS_CMC_BYTE_PAR_COMPRESSION_VARIABLE_HXX
#define LOSSLESS_CMC_BYTE_PAR_COMPRESSION_VARIABLE_HXX
 ...
 */
template <typename T>
class AbstractByteCompressionVariable : public IAMRParallelCompressionVariable<T>
{
public:
    ...
    const std::vector<std::vector<uint8_t>>& GetEncodedPartitionTable() const override {return buffered_partition_table_;}
...
private:
    ...
    std::vector<std::vector<uint8_t>> buffered_partition_table_; //!< Store the levelwise partitioning (directly after coarsening)
...
#endif /* !LOSSLESS_CMC_BYTE_PAR_COMPRESSION_VARIABLE_HXX */
