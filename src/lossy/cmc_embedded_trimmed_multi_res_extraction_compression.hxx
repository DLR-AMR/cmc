#ifndef CMC_EMBEDDED_TRIMMED_MULTI_RES_EXTRACTION_COMPRESSION_HXX
#define CMC_EMBEDDED_TRIMMED_MULTI_RES_EXTRACTION_COMPRESSION_HXX
...
namespace cmc::lossy::embedded::multi_res
{
...
public:
    ...
    MultiResEmbeddedAdaptData(AbstractEmbeddedByteCompressionVariable<T>* variable, const CompressionSettings& settings)
...
GetCoarseApproximationMaximizingResidualsLZC(const VectorView<CompressionValue<T>> values)
{
    ...
    for (auto pred_iter = values.begin(); pred_iter != values.end(); ++pred_iter)
    {
        ...
        const int pred_lzc = cmc::lossy::multi_res::GetCumulativeResidualsLZC<T>(predictor, values);
    ...
    /* Compute the LZC in the residuals for this approximation */
    const int mid_range_lzc = cmc::lossy::multi_res::GetCumulativeResidualsLZC<T>(mid_range, values);
    ...
    /* Compute the LZC in the residuals for this approximation */
    const int mean_lzc = cmc::lossy::multi_res::GetCumulativeResidualsLZC<T>(mean, values);
        ...
        const t8_element_t* elements[], const VectorView<CompressionValue<T>> values)
{
    ...
    for (int idx = 0; idx < num_elements; ++idx)
    {
        ...
        auto [is_approximation_greater, residual, absolute_inaccuracy] = cmc::lossy::multi_res::ComputeMinimalResidual<T>(coarse_approximation, values[idx], permitted_errors, previous_absolute_error);
...
MultiResEmbeddedAdaptData<T>::CollectSymbolFrequenciesForEntropyCoding(const std::vector<CompressionValue<T>>& level_byte_values) const
{
    ...
    for (auto val_iter = level_byte_values.begin(); val_iter != level_byte_values.end(); ++val_iter)
    {
        ...
        /* Update this symbol for encoding */
        IEmbeddedCompressionAdaptData<T>::entropy_coder_->UpdateSymbolFrequency(last_one_bit);       
    }
}
...
MultiResEmbeddedAdaptData<T>::EncodeLevelData(const std::vector<CompressionValue<T>>& level_byte_values) const
{
    ...
    for (auto val_iter = level_byte_values.begin(); val_iter != level_byte_values.end(); ++val_iter)
    {
        ...
        /* Encode the TZC */
        IEmbeddedCompressionAdaptData<T>::entropy_coder_->EncodeSymbol(last_one_bit);
...
CreateMultiResEmbeddedExtractionAdaptationClass(AbstractEmbeddedByteCompressionVariable<T>* abstract_var, const CompressionSettings& settings)
{
...
#endif /* !CMC_EMBEDDED_TRIMMED_MULTI_RES_EXTRACTION_COMPRESSION_HXX */
