#ifndef LOSSY_CMC_EMBEDDED_BYTE_COMPRESSION_SCANLINE_VARIABLE_HXX
#define LOSSY_CMC_EMBEDDED_BYTE_COMPRESSION_SCANLINE_VARIABLE_HXX
...
namespace cmc::lossy::embedded::scanline
{

constexpr bool kStrictlyCompressLevelwise = false;
...
#if 1
//Maybe start with one value per element
template<typename T>
struct ElementData
{
    ...
    uint16_t min_encoding;
    uint16_t max_encoding;
};
...
private:
    ...
    CompressionValue<T> GetDataValueAtIndex(const int tree_id, const int lelement_index) const;
    ...
    //std::vector<CompressionValue<T>> residuals_;
    std::vector<ElementData<T>> extraction_;
...
public:
    ...
    CompressionValue<T> GetDataValueAtIndex(const int tree_id, const int lelement_index) const {return base_variable_->GetDataValueAtIndex(tree_id, lelement_index);}
                                               ...
                                               const t8_scheme_c * ts, const int num_elements, t8_element_t * elements[])
{
    ...
    const ExtractionData<T> extraction = PerformExtraction(forest, which_tree, tree_class, lelement_id, ts, num_elements, elements);

    /* Store the extracted values wihtin the variable */
    base_variable_->StoreExtractedValues(which_tree, lelement_id, num_elements, extraction);
                                                        ...
                                                        const t8_scheme_c * ts, const int num_elements, t8_element_t * elements[])
{
    ...
    const UnchangedData<T> extracted_unchanged_value = this->ElementStaysUnchanged(forest, which_tree, tree_class, lelement_id, ts, num_elements, elements);

    /* Store the unchanged data */
    base_variable_->StoreUnchangedElement(which_tree, lelement_id, extracted_unchanged_value);
    ...
    const t8_locidx_t first_incoming)
{
    ...
    const t8_element_t* elem = t8_forest_get_element_in_tree(coarsened_forest, tree_id, first_incoming);
...
AbstractEmbeddedByteCompressionVariable<T>::StoreResiduals(const t8_forest_t previous_forest, const int tree_id, const int lelement_id, const int num_elements, const ResidualData<T>& residuals)
{
    ...
    std::copy_n(residuals.values.begin(), num_elements, &data_[elem_start_index]);
...
IEmbeddedCompressionAdaptData<T>::CompleteExtractionIteration(const t8_forest_t previous_forest, const t8_forest_t adapted_forest)
{
    ...
    this->base_variable_->ReserveAdaptedRemainingPermittedError(t8_forest_get_local_num_elements(adapted_forest));
...
{
    if constexpr (not kStrictlyCompressLevelwise)
    {
        return true;
    }
...
AbstractEmbeddedByteCompressionVariable<T>::Compress()
{
    ...
    while (adapt_data->IsCompressionProgressing())
    {
        ...
        cmc_err_msg("We break here in order to check the performance of the prediction.");
...
{
    cmc_assert(not extraction_.empty());
    cmc_assert(start_index >= 0 && count >= 0);
    cmc_assert(static_cast<size_t>(start_index + count) <= extraction_.size());

    return VectorView(&extraction_[start_index], count);
}
...
{
    cmc_assert(local_index < extraction_.size());
    return extraction_[local_index];
}

template <typename T>
void
AbstractEmbeddedByteCompressionVariable<T>::StoreExtractedValues([[maybe_unused]] const int tree_id, [[maybe_unused]] const int lelement_id, [[maybe_unused]] const int num_elements, const ExtractionData<T>& extraction)
{
    cmc_assert(tree_id >= 0 && tree_id < t8_forest_get_num_local_trees(mesh_.GetMesh()));

    /* Store the extracted coarse value for the next coarsening iteration */
    extraction_.push_back(extraction.extracted_values);
}

template <typename T>
void
AbstractEmbeddedByteCompressionVariable<T>::StoreUnchangedElement(const int tree_id, const int lelement_id, const UnchangedData<T>& extracted_unchanged_value)
{
    cmc_assert(tree_id >= 0 && tree_id < t8_forest_get_num_local_trees(mesh_.GetMesh()));
    ...
    extraction_.push_back(extracted_unchanged_value.extracted_values);
}
...
{
    cmc_assert(static_cast<size_t>(t8_forest_get_local_num_elements(adapted_forest)) == remaining_permitted_error_.size());
    ...
    cmc_assert(static_cast<size_t>(t8_forest_get_local_num_elements(adapted_forest)) == remaining_permitted_error_.size());
    cmc_assert(static_cast<size_t>(t8_forest_get_local_num_elements(partitioned_forest)) == remaining_permitted_error_new_.size());
...
{
    cmc_assert(tree_id >= 0 && tree_id < t8_forest_get_num_local_trees(adapted_forest));
    cmc_assert(t8_forest_get_tree_element_offset (adapted_forest, tree_id) + lelement_index < t8_forest_get_local_num_elements(adapted_forest));
...
#endif /* !LOSSY_CMC_EMBEDDED_BYTE_COMPRESSION_SCANLINE_VARIABLE_HXX */
