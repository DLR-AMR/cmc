#ifndef LOSSY_CMC_EMBEDDED_BYTE_COMPRESSION_EXTENDED_VARIABLE_HXX
#define LOSSY_CMC_EMBEDDED_BYTE_COMPRESSION_EXTENDED_VARIABLE_HXX
...
namespace cmc::lossy::embedded::extended
{
...
{
    ExtractionData(const CompressionValue<T>& coarse_val)
    : coarse_value(coarse_val) {};
    ExtractionData(CompressionValue<T>&& coarse_val)
    : coarse_value(std::move(coarse_val)) {};
...
{
    UnchangedData(const CompressionValue<T>& coarse_val)
    : coarse_value(coarse_val) {};
...
private:
    ...
    VectorView<CompressionValue<T>> GetViewOnAdaptedData(const int start_index, const int count) const;
    VectorView<CompressionValue<T>> GetViewOnAdaptedData(t8_forest_t adapted_forest, const int tree_id, const int lelement_index, const int count) const;
    CompressionValue<T> GetAdaptedDataValueAtIndex(const int local_index) const;
...
public:
    ...
    VectorView<CompressionValue<T>> GetViewOnAdaptedData(const int start_index, const int count) const {return base_variable_->GetViewOnAdaptedData(start_index, count);}
    VectorView<CompressionValue<T>> GetViewOnAdaptedData(t8_forest_t adapted_forest, const int tree_id, const int lelement_index, const int count) const {return base_variable_->GetView(adapted_forest, tree_id, lelement_index, count);}
    CompressionValue<T> GetAdaptedDataValueAtIndex(const int local_index) const {return base_variable_->GetAdaptedDataValueAtIndex(local_index);}
...
#endif /* !LOSSY_CMC_EMBEDDED_BYTE_COMPRESSION_EXTENDED_VARIABLE_HXX */
