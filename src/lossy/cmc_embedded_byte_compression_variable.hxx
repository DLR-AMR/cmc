#ifndef LOSSY_CMC_EMBEDDED_BYTE_COMPRESSION_VARIABLE_HXX
#define LOSSY_CMC_EMBEDDED_BYTE_COMPRESSION_VARIABLE_HXX
...
public:
    ...
    {
        if (not is_inaccuracy_tracking_activated_) {return 0.0;}
        cmc_assert(inaccuracy_storage_ != nullptr);
...
protected:
    ...
    void SetupInaccuracyTracking(const size_t size_hint = kInvalidSizeHintForInaccuracyContainer) {
        if (not is_inaccuracy_tracking_activated_) {return;}
...
private:
    ...
    void SwitchDeviationsForInaccuracyTracking() {
        if (not is_inaccuracy_tracking_activated_) {return;}
        cmc_assert(inaccuracy_storage_ != nullptr); inaccuracy_storage_->SwitchDeviations();
    }
    ...
    bool is_inaccuracy_tracking_activated_{true};
};
...
template <typename T>
double
IEmbeddedCompressionAdaptData<T>::GetPreviousAbsoluteError(int tree_id, int lelement_index)
{
...
template <typename T>
inline void
AbstractEmbeddedByteCompressionVariable<T>::RepartitionInaccuracyTracking(t8_forest_t adapted_forest, t8_forest_t partitioned_forest)
{
...
#endif /* !LOSSY_CMC_EMBEDDED_BYTE_COMPRESSION_VARIABLE_HXX */
